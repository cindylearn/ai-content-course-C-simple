...
> **步骤顺序（照这个走，前面没齐别放行）：**
> **第 0 步** 底座（课前：装 Claude App → 装 git → 注册账号 → 接 MCP；skill 课上才装）→ **第 1 步** 落脚点（东西存哪）→ **第 2 步** 新/旧业务？→ **第 3 步** 建业务 md → **第 4 步** 品牌名（其余品牌地基交给 `01-branding`）→ **第 5 步** Notion 骨架 → **放行**：先用 `01` 定品牌地基 → 再做第一条内容。
...
### 第 4 步 · 品牌地基（🔴 交给 `01-branding` 做，00 不重复问）
🔴 **品牌地基 = `01-branding` 的活，00 不再内嵌那 7 题**（否则两个 skill 撞、改一处另一处漂）。00 这里只做两件小事、够搭 Notion 骨架就行：
1. **品牌名**（建 folder / Notion 命名要用）
2. **一句话说清生意做什么**（一句就够）
→ 写进业务 md。

- **其余全归 01**（00 别问）：受众 / persona / 合规红线 / 命名格式 / 两条内容路线 / **logo（深色·浅色·透明 PNG 三种）** / 内容角度（6 大人性需求）。
- **引导去 01**：搭好骨架后说「地基环境好了，接下来用 **`01-branding`**（斜杠 `/suma-ai-01-branding`，或说「帮我定品牌定位」）一题一题把品牌地基定好。」
- ⚠️ **课前只装了 00（01 还没装）→ 就到这为止，告诉学员『品牌地基明天课上用 01 做』，别卡住。**
...
### 放行 → 先用 `01-branding` 定品牌地基，再做第一条内容
以上第 0–5 步齐了（环境 + 骨架好、品牌名有了）→ 🔴 **先说「地基环境 OK，接下来用 `01-branding` 把品牌地基定好（受众 / persona / 红线 / 命名 / logo / 两条内容路线 / 内容角度）」**——品牌地基是 01 的活，00 不在这问。
品牌地基定好后，再**一次一题**问做第一条内容（选项卡）：
1. **做哪个受众？**（用 01 定好的受众）
...
→ 选【文案+视频】：同一条立项页里 `02-copy` 写文案/脚本 ＋ `04-video-avatar` 出片（要锁同一个人物再 `05`）。
