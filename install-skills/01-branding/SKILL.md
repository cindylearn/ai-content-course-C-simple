...
- **logo（3 种版本）—— 🔴 先读业务 md，别重复问**：logo 应该在 `00 守门人` 就问过、记在业务 md（深色版 / 浅色版 / 透明 PNG）。**业务 md 已有 → 一句确认「你的 logo 我用记好的这几个」，不要再问一遍**。
  - **只有业务 md 没记**（例如单独跑 01、没走过 00）→ 才问这一次（一题一张卡）：① **深色版**（放浅色背景）② **浅色 / 白色版**（放深色背景）③ **透明背景 PNG**（最关键，没透明底合成带白框）。问完写回业务 md。
