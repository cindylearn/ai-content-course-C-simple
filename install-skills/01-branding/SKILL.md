...
description: >-
 SUMA AI 内容系统 · 第 01 步 —— 从零搭系统的起点（装机已在 00 做完）。先打地基：① 定落脚点（成品 folder + 用哪个 Notion + Drive folder）② 建业务档案（业务 md，记品牌名/一句话生意）③ 搭 Notion 大脑（内容矩阵骨架）→ 再定品牌地基：受众 / persona / 红线 / 命名 / logo / 两条内容路线（个人IP vs 卖货）。
 触发词：从零搭系统 / 开始 / 落脚点 / 搭 Notion 大脑 / 品牌定位 / 定受众 / 品牌地基 / 命名格式 / 红线。
 跟用户对话一律用中文；教练式一题一题问。
metadata:
 type: reference
...
# suma-ai-01-branding —— 从零搭系统的起点：先打地基（定落脚点 → 建业务档案 → 搭 Notion 大脑骨架）→ 再定品牌地基（受众 / persona / 红线 / 命名 / logo / 两条内容路线：个人IP vs 卖货）。装机在 00 已做完。
...
| ② First Prompt（说一句话）| **「帮我从零搭系统」**（同义：帮我从零搭我的 AI 内容营销系统 / 帮我定品牌定位、受众、红线）|
...
## 🛑 01 只做什么（总纲 · 先读这条，压过下面一切「生成」话术）

> 🔴🔴 **01 的产出只有三样：① 业务 md ② Notion 大脑骨架（内容矩阵）③ 品牌地基（受众/红线/命名/logo…）。做完 → 交接（Step 6）。**
> **01 绝不生成任何内容**：❌ 不写文案 ❌ 不出图 / 编图 prompt ❌ 不做视频 ❌ 不问「哪个漏斗层」 ❌ 不建「某条内容」的立项页去写文案。
> 🔴 下面正文里若出现「brainstorm 完 → 生成 → 回填『文案』列 / 贴『成品』块」「进对应 skill 写 / 生成」这类**通用话术，是给内容 skill（02–07）的，在 01 里一律不适用** —— 那些活到 Step 6 **交接给 `02-copy` / `03-image` / `04-video`**，由它们一题一题问、由它们写 / 生成。**选完「海报 / 视频」就停下、触发对应 skill，绝不自己往下做**（这是最常犯的 bug）。
...
- 本地成品 folder（业务 md 在最外层）：`~/我的AI内容营销/<品牌>` —— 🔴 里面结构**写死**：`<漏斗层 ToFu/MoFu/BoFu>/<行业受众 或 campaign>/<序号>-<STAGE>-<受众>-<标题>`（详见 ①定落脚点「📁成品 folder 结构」；Drive 同结构镜像）
...
## 🔴 Step 0 · 先打地基（01 从零搭系统的第一步 —— 品牌地基 7 题之前先做）

> 装机（00）做完了。01 一开场先把「地基」打好（落脚点 / 业务档案 / Notion 大脑），再进下面的品牌地基 7 题。**这些以前在 00，现在全归 01。**

**🌐 开场第一题（先于一切）· 内容语言：** 🔴 跟学员对话你一律用中文；这题只定**出的内容**（文案 / 图内字 / 台词）用哪种语言。一题一张卡「你要出的**文案 / 台词**用哪种语言？」→ ① 🇲🇾 **马来西亚华文**（中文为主，掺一点 English / Malay / 方言，最本地）【默认】· ② 规范中文 · ③ English · ④ Bahasa Malaysia · ⑤ 中英 Rojak / 方言混（共 5 个 · 卡最多 4 → 第 5 放「其他」）。**选定 = 出的内容都用它（对话仍中文）**，写进业务 md；之后每个 skill 一句确认。**答完这题，才进下面 ① 定落脚点。**

**① 定落脚点（东西存哪 · 先给他一个家）**
1. **成品 folder** —— 默认 `~/我的AI内容营销/<品牌名>`（例 `LeadGenz`），你来建好。🔴 **建好后立刻在里面一次建好顶层「漏斗层」3 个子文件夹（教学统一 · 名字/顺序写死）：`ToFu` · `MoFu` · `BoFu`**（业务 md 放最外层）。再往下的「行业受众 / campaign」层由各 skill 归档时按写死结构建（见下方「📁 成品 folder 结构」）。
   > 🖱️ 要路径就教他复制：Mac 点选 folder 按 `Option + Command + C`；Windows 右键 `Copy as path`。
2. **哪个 Notion** —— 待会在里面搭骨架。
3. **（可选 · 要自动归档到 Drive 才做，起步可跳过）rclone 连 Drive**：要他的 Drive 成品 folder 链接（`/folders/` 后那段 = folder ID，记进业务 md）→ 一条命令 `rclone config create gdrive drive scope=drive`（Claude 直接替他跑、答案已预填，他只需最后弹浏览器点「允许」）→ 验证 `rclone lsd gdrive:`。**配好前别开始出内容。**
4. 落脚点定了 → 写进业务 md。

> 📁 **成品 folder 结构（写死 · 教学统一 · 每个 skill 归档都照这个建下去 · 顺序 = 漏斗层 → 行业受众/campaign → 编码文件名）：**
> ```
> ~/我的AI内容营销/<品牌>/                    ← 例 LeadGenz
> ├─ 业务-<品牌>.md                           ← 最外层
> ├─ ToFu/   MoFu/   BoFu/                    ← 第1层 漏斗层（01 一次建好这 3 个）
> │    └─ <行业受众 或 campaign>/             ← 第2层（如 补习中心 / 美业 / 开学季招生）· 归档时没有就建
...
**③ 建「业务 md」** 没有 `业务-<品牌名>.md` 就先建一个，之后每步往里记（先记品牌名 + 一句话生意）。

**④ 搭 Notion 大脑骨架（走文案前必须先做，学生最爱跳过 —— 不给跳）：**
```
📁 <品牌>（大脑）
├── 🧱 品牌地基 ├── 📋 内容矩阵(database) ├── 🎨 素材库 └── ✅ 成品归档
```
在「内容矩阵」建 database —— **照本 skill 上方「一、database 属性」建全部属性列**（顺序照 default view；**Title = 内容角度**，不是命名标题）。只搭 database + 属性 + 空模版结构，别填内容；**教学员在 Notion「设为默认模版」**（API 设不了，手动点一次）。表格 `Higgsfield Prompt` 列宽 ≈ 562px。

> 🎯 **搭好后把「目的地」写回业务 md**：内容矩阵 database 链接/id + dashboard 链接 + 第①步的 Drive folder ID —— 这几样 = 这个业务的**落脚点 / 目的地**，之后每个 skill（顺跑或单独跑）都靠业务 md 这几个链接连回 Notion + Drive。

→ 地基打好，进下面 **品牌地基 7 题**。

## 🧱 品牌地基（教练**一题一题**问你 —— 一次一个，答完再问下一个）

> 🔴 **铁律：下面每一项都单独问一题、等他答完才问下一项 —— 哪怕是打字题（品牌名 / 生意 / CTA），也绝不说「麻烦一次告诉我」「几个基本信息一起给我」把多个信息一次要。一次一个。** 课前先想好，当天就不卡。
...
> 🎯 **定位铁律（打地基时就锁进业务 md · 后面所有 skill 都读它）：明确三层 —— ① 谁在卖 = user · ② 卖什么 = user 的服务/产品 · ③ 卖给谁 = 买家。** 🔴 服务型生意（agency / 自由职业 / 顾问 / 教练）要写清"**我提供什么服务、交付什么结果**"，别只写买家的行业。业务 md 加一行「**服务品类画面信号**」＝出图/视频时主视觉该放什么（营销代运营→社媒后台/内容日历/成品帖/投放 dashboard；设计→作品集；顾问→策略白板/成果数字；电商→产品特写；补习/美业只有 user 本人是这行才放教学/美容），让 03/04 出的东西"**一眼看得出卖什么服务**"、不滑成买家行业。买家只定称呼 / 痛点。
...
> 2. **地基 / 落脚点还没定？** 没关系 —— **这就是 01 的第一步**（01 ＝ 从零搭系统的起点）：先带用户 ① 定落脚点（成品 folder + 用哪个 Notion + Drive folder）② 建业务档案（业务 md，记品牌名 / 一句话生意）③ 搭 Notion 大脑（内容矩阵骨架，照下方模版「一、属性」建库）→ **再**进下面的品牌地基 7 题。**别把用户支回 00**（00 只管装机）。
> 3. **Notion 大脑骨架**：dashboard + folder（品牌地基/内容矩阵/素材库/成品归档）+ 内容矩阵 database —— 没有就**在这一步现场建好**（这是 01 的活，不是回 00）。
> 4. 🎯 **连上目的地：** 建好后把落脚点**写回业务 md** —— **哪个 Notion 内容矩阵（DB 链接/id）＋ 哪个 Drive 成品 folder（folder ID）**，之后每个 skill 开工「前置闸门第 4 条」都来这读、连回 Notion + Drive。
> **缺装机 / 连接器 → 回 `suma-ai-00-setup`；地基缺 ＝ 01 现场补（就是本 skill 该做的事），别硬跳过、别循环支回 00。**
...
## 🧭 定品牌地基 · 照 Step 1–5 走（80% 的结果由这 20% 决定 —— 方向先对，别急着生产）

> 帮用户把「品牌地基」一次性定下来，写进 memory + 业务 md；之后所有 part（图/文案/视频/数字人/PPT/配乐）都先读这份地基才动手。
> **八二法则**：品牌定位 = 用户的「原始资料库」，喂错了，之后每张图 / 每条文案 / 每支视频都是**精美但没用**。问完写进 memory，才放行做内容。

### Step 1 · 先搞清「这生意怎么赚钱」（决定所有内容角度）
问清它**卖什么、怎么交付** —— 不同商业模式角度完全相反：
- **代运营 / agency**（帮客户做 = done-for-you）→「你太忙 / 做不好，**交给我们**」，绝不说「给你一套自己搭」
- **卖产品 / 电商** →「这产品解决你什么问题」
- **服务店**（美容 / 补习 / 餐饮，到店消费）→「来店体验 / 效果 / 口碑」
- **教 / 卖方法**（像 SUMA）→「教你自己会做」
⚠️ 内容角度必须从「这业务自己怎么赚钱」长出来，**绝不 import SUMA 的角度 / 产品 / 卖点**（别的业务照搬 = 烂成品）。

### Step 2 · 品牌地基（🔴 一题一题问 —— 一次一个，答完再问下一个；每题先给范例再问，问完复述确认。**下面每一条都单独一题，含打字题也绝不「一次告诉我几个」**）
1. **一句话说清生意 / 品牌做什么**（例：SUMA =「教人用 AI 做营销内容的课程」）
2. **品牌名叫什么？**（单独一题）
3. **主打的产品 / 服务是什么？**（单独一题）
4. **内容最后把人导去哪？（CTA 链接）**（报名页 / WhatsApp / 官网 · 单独一题）
5. **目标受众 3–5 个**（🔴 别只丢一两个空泛例子 —— 用下面这张「分类菜单」帮他挑**准**，一次挑 3–5 个、可跨类；每挑一个再追一句「具体是哪一种？」缩到最具体那层）：
   > 🧑‍💻 **接案 / 自由职业**：接案 Marketer · 设计接案 · 影片剪辑接案 · 文案接案 · 顾问型个人 IP
   > 🛒 **电商 / 自有产品**：网店老板 · 自有品牌主理人 · 包装 / 罐装食品 · 美妆护肤品牌
   > 💇 **到店服务（美业 / 餐饮）**：美容院 / 美发 / 纹绣 / SPA 老板娘 · 餐饮店
   > 🏫 **教育 / 机构**：补习老师（SPM 等）· 培训机构 · 课程主理人 · 机构内容负责人 / 派员工的老板
   > 🏠 **专业服务**：房产 agent · 保险 / 理财顾问 · 律师 / 会计
   > 🧘 **身心灵 / 疗愈**：音疗 · 瑜伽 · 身心灵导师
   > 👔 **企业角色**：老板 / 中小企业主 · in-house Marketer（公司里什么都要你做的你）· Sales
   > 👉 一题一张卡（按分类给他挑）；想不出 → 帮他 brainstorm。
6. **每个受众逐个深挖 persona —— 🔴🔴 全部做成选项卡，别让用户空手打字（很多人答不出）**。做法：**Claude 用 **CMO 战略官**的判断、按「这个受众 + 他的生意」**逻辑推导**一版**，每项给 **3–4 个常见选项 + 「其他(自己打)」+「✅ 就用这版」**，用户只要点。一项一张卡，问完复述整张 persona 卡。7 项 + 每项的默认选项：
   > ① **一句话是谁**（Claude 直接拟 2–3 句候选让他选，例「自己投广告的网店老板」/「公司什么都要你做的 Marketer」/「刚接管家里补习班的二代」）
   > ② **年龄段**：18–25 / 26–35 / 36–45 / 46+ ｜ **性别倾向**：偏女 / 偏男 / 都有
   > ③ **最痛的一件事**（→ ToFu 钩子 · 给菜单挑 1–2）：没时间做内容 / 不会设计做图 / 不会写文案 / 请人・外包太贵 / 发了没人看 / 不会投广告 / 客源不稳定 / 客人嫌贵 / 其他
   > ④ **他想要的结果**（→ offer 角度 · 挑 1–2）：更多询问・客户 / 省时间 / 省外包钱 / 建立专业形象 / 涨粉被看见 / 卖更多货 / 其他
   > ⑤ **常出没平台**（可多选）：FB / IG / 小红书 / TikTok / WhatsApp ｜ **场景**：办公室 / 自家店里 / 在外面跑 / 在家
   > ⑥ **预算敏感度**：高（很在意价）/ 中 / 低（有效果就买）｜ **最大顾虑**：怕没效果 / 怕自己学不会 / 怕花时间 / 怕被骗 / 其他
   > ⑦ **外形 / 出现画面**（做图 / 视频用）：**Claude 直接按受众拟一版**（🔴 一律**马来西亚本地人**，例「30 出头女老板娘・淡妆・自家美容院」），给他「✅ 用这版 / 微调 / 换一版」。
   > 👉 写进业务 md：**每个受众 ＝ 一张 persona 卡（①–⑦）**。之后所有图 / 文案 / 视频都照它，换受众就换这张卡。
   > 🔴🔴 **核心原则（每一题都守）：① 能给选项就给选项，绝不丢开放题让他卡住；② 选项要「按他的生意定制推荐」，不是套通用清单** —— 例如他开美容院，③最痛的点就给「淡季没客 / 老客不回头 / 拍不出好看的照片 / 不会做促销贴」这种**美业专属**的；他做补习，就给「招生难 / 家长比价 / 排课乱 / 没时间发帖」。上面的通用菜单只是**兜底**，Claude 要先**读业务 md、用 CMO 战略官对这一行的判断（痛点／动机／persona 分析）逻辑推导出专属选项让他点**，判断不准他再选「其他」。🔴 你是本 skill 开头设定的 **CMO 战略官** —— 用你对这一行的专业判断把选项**推导**出来，不是瞎猜。
7. **合规红线：哪些词不能用**（例：不写价 / 不写「免费」/ 不写名额数字 → 用「名额有限」；品牌名要完整）
8. **内容路线（核心分叉，必须选一条，否则角度飘）**：① **个人 IP** — 观点 + 持续输出建信任 → 数字人口播讲观点；② **卖产品 / 服务** — 直接推货成交 → 卖货文案 / 产品广告
9. **命名格式（选一个，之后全统一）**：推荐编号式 `[编号] [钩子]-[Stage]-[受众]：[标题]`（例 `03 痛点直击-ToFu-服装老板娘：换季断码.png`）；或日期式 / 平台式 / 极简式。文件名里 `/` 一律转全角 `／`。

### Step 3 · 准备素材（唯一要用户自己准备的真东西）
- **logo（3 种版本）—— 🔴 先读业务 md，别重复问**：logo 是**本 skill（01）**问的（不在 00，00 只装机）；若业务 md 已记（深色版 / 浅色版 / 透明 PNG）**→ 一句确认「你的 logo 我用记好的这几个」，不要再问一遍**。
  - **业务 md 没记**（第一次跑 01）→ 现在问这一次（一题一张卡）：① **深色版**（放浅色背景）② **浅色 / 白色版**（放深色背景）③ **透明背景 PNG**（最关键，没透明底合成带白框）。问完写回业务 md。
  - **没有 / 只有一种** → 记「logo 待补」，用**文字品牌名**顶着；提醒做齐 3 种再回来。
  - 🔴 **logo 绝不让 AI 画**（会出假 logo/乱字），一律成品后期合成。
- **品牌色 hex + 字体名**（例 `#28c2e7` 青 / `#0f75bc` 蓝 / `#fcb040` 橙）。
- ⚠️ 不用准备产品照片 —— 画面全 AI 生成。

### Step 4 · 定内容角度：6 大人性需求 + 组装公式
每条内容配一个主需求，一批 6 种轮着来（不腻、覆盖全）：
- **确定感** → 教程/清单/对比 · **新鲜感** → 反直觉/趋势/幕后 · **重要感** → 身份认同/专属限量
- **爱与连接** → 社群/共鸣故事 · **成长** → 转变前后/技能拆解 · **贡献** → 使命/帮助他人
- 搭配：确定感 / 成长 → 最搭 MoFu；新鲜感 / 重要感 → 最搭 ToFu；爱与连接 / 贡献 → 全漏斗，尤其 BoFu 临门。
**组装公式**：`受众 × 漏斗层(ToFu/MoFu/BoFu) × Hook × 人性需求 = 一格一条内容`。Hook 谱：痛点直击 / 对比反差 / 趋势警醒 / 行业揭秘 / 实操演示 / 客户见证 / 算账 ROI / 行动 CTA（换成用户行业说法）。

### Step 5 · 收尾：写进 memory + 业务 md
7 题问完 → 整理成品牌地基（一句话生意 / CTA / 受众 + persona / 红线 / 内容路线 / 命名格式 / 品牌色 + logo）写进 memory + 业务 md。之后每次开工先读它。（详见「参考 · 定制模板（填空版）」+「参考 · 6 大人性需求」）

### Step 6 · 放行 → 🚦 交接给下一个 skill（🔴 01 到此为止，绝不代做别的 skill 的活）

地基都定好了 → 问用户想先做哪种第一条内容。**用选项卡问，只给这两个「成对」选项**（外加「先休息」）：
- **① 文案 + 图文海报**（`02-copy` 写文案 ＋ `03-image` 出图）
- **② 文案 + 口播视频**（`02-copy` 写文案/脚本 ＋ `04-video-avatar` 出片）

🔴🔴 **选完就停 —— 交接，绝不自己接着做。** 写文案 / 出图 / 出视频的**全套提问流程、方法、版式库、大师都在各自的 skill 里，01 里没有**。你若留在 01 里硬做，就会变成**自己瞎决定主题 / 设计 / 内容（错，这正是要避免的 bug）**。正确做法 ＝ 让用户**装好 ＋ 触发**对应 skill，由那个 skill **一题一题问**（受众 / 版式 / 风格 / 图内文字 / 台词…）：

> **选 ① 海报：** 先说「**帮我写文案**」→ `02-copy` 接手；文案定稿后说「**帮我出图**」→ `03-image` 接手（它会问版式 / 风格 / 受众 / 图内文字…全部设计问题）。
>
> **选 ② 视频：** 先「**帮我写文案**」（`02-copy`）→ 再「**做口播视频**」→ `04-video-avatar` 接手。
>
> 💡 **整包 01–08 已一次过装好**（课上一次装完）—— 这里**直接说触发词就启动**，不用再装；万一漏装了某个，先补装 `…/install-skills/0X` 再触发。
>
> 🔴 装好那个 skill 后若打斜杠显示「Unknown command」＝ **重开一次 Claude**，或**直接说那句自然语言触发词**（立刻通）。
> ✅ **触发时机两种都行**：① 地基答完后，**直接在同一个对话说下一个触发词**（如「帮我写文案」）接着做；② 或进另一个 session / 换个时候 **单独触发**。**关键是每一步那个 skill 都要被「触发」才会一题一题问你**——没触发就别留在 01 里替它做。

🔴 **成对铁律**：**文案永远跟图或视频成对做** —— 绝不单独做文案、也绝不做没有文案的图/视频（一条内容 ＝ 一句发帖 caption ＋ 一个视觉）。别把「文案 / 海报 / 视频」拆成三个独立选项。

🔴 **交接铁律（这一条最重要，别再犯）**：01 的职责**只到「定好地基 ＋ 把用户交接到对应 skill」为止**。**绝不**在 01 里替 `02/03/04` 写整篇文案、编图 prompt、决定海报设计/主题/风格、生成图或视频 —— 那些是对应 skill 的活，**它们各有整套提问 + 版式库 + 大师，01 里根本没有**。**没装好、没触发那个 skill，就绝不代它做事**（宁可停下让用户去装，也不要自己瞎发挥）。
🔴 选完 → **立刻交接、别自己往下做**：直接触发 `02-copy`（跟用户说「帮我写文案」）。**漏斗层（ToFu/MoFu/BoFu）、立项页、Status、文案，全部由 `02-copy` 去问 / 建 / 写，不是 01**（01 里没有那套提问流程）。🔴 **01 绝不自己问漏斗层、绝不自己建立项页写文案、绝不自己出图 prompt** —— 到「交接」就停。数字人 / PPT / 配乐是**配件**，需要时在对应 skill 的流程里带出，不当第一条的独立选项。