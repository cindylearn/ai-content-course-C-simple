...
- **先问：有没有品牌 logo？**（一题一张卡）
  - **有** → 要他给 **3 种版本**（成品合成要按背景自适应用）：
    ① **深色版 logo**（深色图案，放**浅色背景**用）
    ② **浅色 / 白色版 logo**（放**深色背景**用）
    ③ **透明背景 PNG**（最关键，没有透明底就会带白框，合成难看）
  - **没有 / 只有一种** → 先记「logo 待补」，用**文字品牌名**顶着；提醒他做齐这 3 种再回来（或转 `03-image` 顺手帮他排一个简单文字 logo）。
