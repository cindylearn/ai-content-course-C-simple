...
> **一次只问一题，用可点选项卡（`AskUserQuestion`），等答完再问下一题——绝不把下面几题一次全倒给用户。** 能做成选项的题（行业/痛点/KPI/受众）就做选项卡 —— 🔴 **按他的生意先猜 3–4 个专属选项 +「其他」**（不是套死通用清单，例美容院·痛点→淡季没客／老客不回头），只有「生意一句话介绍」这种真开放题才让他打字。
...
6. **每个受众逐个深挖 persona —— 🔴🔴 全部做成选项卡，别让用户空手打字（很多人答不出）**。做法：**Claude 先按「这个受众 + 他的生意」猜一版**，每项给 **3–4 个常见选项 + 「其他(自己打)」+「✅ 就用你猜的」**，用户只要点。一项一张卡，问完复述整张 persona 卡。7 项 + 每项的默认选项：
   ...
   > 🔴🔴 **核心原则（每一题都守）：① 能给选项就给选项，绝不丢开放题让他卡住；② 选项要「按他的生意定制推荐」，不是套通用清单** —— 例如他开美容院，③最痛的点就给「淡季没客 / 老客不回头 / 拍不出好看的照片 / 不会做促销贴」这种**美业专属**的；他做补习，就给「招生难 / 家长比价 / 排课乱 / 没时间发帖」。上面的通用菜单只是**兜底**，Claude 要先**读业务 md、按他这行先猜一版专属选项让他点**，猜不准他再选「其他」。
