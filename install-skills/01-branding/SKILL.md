...
- 本地成品 folder（业务 md 在最外层）：`~/我的AI内容营销/<品牌>` —— 🔴 里面结构**写死**：`<内容类型 01-05>/<主题>/<漏斗层>_<受众>/<序号>-<STAGE>-<受众>-<标题>`（详见 ①定落脚点「📁成品 folder 结构」；Drive 同结构镜像）
...
**① 定落脚点（东西存哪 · 先给他一个家）**
1. **成品 folder** —— 默认 `~/我的AI内容营销/<品牌名>`，你来建好。🔴 **建好后立刻在里面一次建好顶层「5 个内容类型」子文件夹（教学统一 · 名字/顺序写死）：`01-文案` · `02-图文海报` · `03-视频` · `04-PPT` · `05-音乐`**（业务 md 放最外层）。再往下的「主题 / 漏斗层_受众」两层由各 skill 归档时按写死结构建（见下方「📁 成品 folder 结构」）。
...
> 📁 **成品 folder 结构（写死 · 教学统一 · 每个 skill 归档都照这个建下去 · 顺序 = 内容类型 → 主题 → 漏斗层_受众 → 文件）：**
> ```
> ~/我的AI内容营销/<品牌>/
> ├─ 业务-<品牌>.md                         ← 最外层
> ├─ 01-文案/ 02-图文海报/ 03-视频/ 04-PPT/ 05-音乐/   ← 第1层 内容类型（01 一次建好这 5 个）
> │    └─ <主题或活动>/                     ← 第2层 主题/campaign（如 开学季招生 / 日常内容）· 归档时没有就建
> │         └─ <漏斗层>_<受众>/             ← 第3层（如 BoFu_打工人）· 归档时没有就建
> │              └─ <序号>-<STAGE>-<受众>-<标题>.<后缀>   ← 文件名编码
> ```
> 例：`03-视频/开学季招生/BoFu_打工人/001-BoFu-打工人-一天出片.mp4`。
> 🔴 **01 打地基时只一次建好顶层 5 个类型 folder**；「主题」「漏斗层_受众」是动态的，**各 skill 归档时按上面结构自动建（没有就建、名字照写死格式），别乱放**。🔴 **Drive 成品 folder 里 rclone 归档也建同一套结构**（镜像）。
...
### Step 6 · 放行 → 做第一条内容（🔴 成对，别拆成三个独立选项）
地基都好了 → 带用户做第一条内容。**用选项卡问，只给这两个「成对」选项**（外加「先休息」）：
- **① 文案 + 图文海报**（`02-copy` 写文案 ＋ `03-image` 出图，同一条立项页）
- **② 文案 + 口播视频**（`02-copy` 写文案/脚本 ＋ `04-video-avatar` 出片，同一条立项页）

🔴 **文案永远跟图或视频成对做** —— **绝不单独做文案、也绝不做没有文案的图/视频**（一条内容 ＝ 一句发帖 caption ＋ 一个视觉）。**别把「广告文案 / 海报 / 口播视频」拆成三个独立选项让用户选**（那样会做出没配图的孤文案、或没文案的孤图）。
