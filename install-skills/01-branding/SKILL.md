...
### Step 2 · 品牌地基 7 题（一题一张卡，一题一题问，每题先给范例再问，问完复述确认）
1. **一句话说清生意 / 品牌做什么**（例：SUMA =「教人用 AI 做营销内容的课程」）
2. **品牌名 / 主打产品 / CTA 链接**（内容最后把人导去哪：报名页 / WhatsApp / 官网）
3. **目标受众 3–5 个**（例：内容创作者、服装老板娘、美妆美容院老板娘、补习老师；想不出 → 帮他 brainstorm）
4. **每个受众的 persona：气质 / 出现场景**（人物一律**本地人**，含手机屏内 / 模特 / 背景）
5. **合规红线：哪些词不能用**（例：不写价 / 不写「免费」/ 不写名额数字 → 用「名额有限」；品牌名要完整）
6. **内容路线（核心分叉，必须选一条，否则角度飘）**：① **个人 IP** — 观点 + 持续输出建信任 → 数字人口播讲观点；② **卖产品 / 服务** — 直接推货成交 → 卖货文案 / 产品广告
7. **命名格式（选一个，之后全统一）**：推荐编号式 `[编号] [钩子]-[Stage]-[受众]：[标题]`（例 `03 痛点直击-ToFu-服装老板娘：换季断码.png`）；或日期式 / 平台式 / 极简式。文件名里 `/` 一律转全角 `／`。
