...
6. **每个受众逐个深挖 persona（🔴 一个受众一组子题，绝不用一句「气质 / 场景」带过）** —— 对**每一个**选中的受众问清这 7 项（一项一题，问完复述）：
   > ① **一句话是谁**（角色 + 行业，例「自己投广告的网店老板」「公司里什么都要你做的 Marketer」）
   > ② **年龄段 + 性别倾向**（例 late-20s～40 出头 · 偏女 / 偏男 / 都有）
   > ③ **最痛的一件事**（他每天最烦、最怕的那个点 → 决定 ToFu 钩子）
   > ④ **他想要的结果**（买你的东西是为了达成什么 → 决定 offer 角度）
   > ⑤ **常出没的平台 / 场景**（FB / IG / 小红书 / TikTok · 办公室 / 店里 / 在外面跑）
   > ⑥ **预算敏感度 + 最大购买顾虑**（贵不贵是不是问题、最容易卡在哪 → 决定 BoFu 消顾虑）
   > ⑦ **外形 / 出现画面**（做图 / 视频用 —— 🔴 人物一律**马来西亚本地人**，含手机屏内 / 模特 / 背景）
