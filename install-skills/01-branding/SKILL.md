...
> **选 ① 海报：** 先跟用户说这句 → 「**装 `02-copy` → 触发『帮我写文案』**」，链接 `https://github.com/cindylearn/ai-content-course-C-simple/tree/main/install-skills/02-copy`。文案定稿后再 → 「**装 `03-image` → 触发『帮我出图』**」，链接 `…/install-skills/03-image`（03 会问你版式 / 风格 / 受众 / 图内文字…全部设计问题）。
>
> **选 ② 视频：** 先文案（同上 `02-copy`）→ 再 → 「**装 `04-video-avatar` → 触发『做口播视频』**」，链接 `…/install-skills/04-video-avatar`。
>
...
选完 → 先确认受众（用 Step 2 定好的）→ 再问漏斗层（ToFu / MoFu / BoFu）→ 照 Notion 模版建这条立项页、Status 起步 → 进对应 skill 写 / 生成。数字人 / PPT / 配乐是**配件**（需要时在流程里带出），不当第一条的独立选项。