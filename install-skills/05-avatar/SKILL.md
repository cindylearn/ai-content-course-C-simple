...
## 🎬 做数字人 · 照 Step 1–4 走（一步做完再下一步）

### Step 1 · 先问「这个数字人怎么来？」（一题一张卡）
**Q1. 要哪种进阶数字人？（二选一）**
- **A · 九宫格数字人** — AI 凭空生成、选一个锁定（不是真人本尊）→ 进 Q2 选 model + Step 2 九宫格选脸。性别/年龄/气质照目标受众定。出视频画面+声音一起 prompt（`generate_audio=true`），不用分开。
...
2. **语音样本（关键）** 一段干净录音 ~1 分钟（安静、近麦 ~15cm、无背景乐、连续说话）→ `create_voice_from_confirmed_audio` 克隆；样本脏 → 克隆薄带杂音。
3. **可选·一小段说话视频（几秒）** 参考神态/口型，更像本人。

**Q2.（选 A 才问）用什么 model 出人物图？** 九宫格首选 **`GPT Image 2`**（Higgsfield `generate_image` 里的一个 model，不是另开 ChatGPT，出人脸最准最一致）。给选项时把 GPT Image 2 放第一（推荐），备选 soul_2 / nano_banana 等。id 别自己编，用 `models_explore` 查当前值；查不到就让学生在 Higgsfield 网页 model 下拉里选。

### Step 2 · （选 A）九宫格选脸 → 定妆照锁脸（两步，中间停下等号码）
**① 九宫格选人**（用 Q2 选的 model 出；1:1 正方，一张图 9 个不同的人）：
```
A single square 1:1 casting board arranged as a clean 3x3 grid of 9 passport-style
headshots of 9 DIFFERENT [目标市场] [men/women], all [age range] with a [vibe] look.
IMPORTANT: each of the 9 is a DIFFERENT individual with a DIFFERENT face — different
...
```
`[目标市场]/[men/women]/[age range]/[vibe]` 照目标受众填；人物一律本地人。**9 个不同的人，不是同一张脸换 9 个发型** —— 先把五官写死再写头发。9 张脸清单列给用户看。⏸️ 停，等用户回一个号码（1–9）。
...
- **照片级真人**：`PHOTOREAL, REALISTIC HUMAN presenter, a real-looking [本地] person, NOT a cartoon, NOT a 3D avatar`。
- **人物一律本地** + 贴行业气质（美妆老板娘=年轻好皮肤淡妆、不要 aunty）；参考图具体到特征（脸型/眼镜/衣着/痣），换镜不换人。
- 屏幕里的主播也**不要乱码字**。