...
> 🔴🔴 **强制用模版立项（单独跑这个 skill 也一样，绝不跳过）：** 动手写 / 生成**之前**，先在 Notion「内容矩阵」database **New 一条立项页**，正文**照 `05-Notion内容矩阵页模版` 填**（按媒体类型选对应结构：概念 / 文案 / 图内文字层级 / prompt 表 / 台词…）；产出完再**回填**（文案进「文案」列、成品图贴「成品」块、改 Status）。
> 🔴 **绝不**：不建页就直接产出、建个空白页、把内容散着丢在对话里。**每一条内容 ＝ 内容矩阵里的一行 ＋ 一个照模版填好的立项页**。🔴 没有「内容矩阵」database？先回 `suma-ai-00-setup` 建。模版全文见本 skill 下方参考资料。
