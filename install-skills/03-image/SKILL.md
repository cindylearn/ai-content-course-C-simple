...
>
> 🎣 **大标题怎么写 = 用钩子库**（见 `02-《AI爆款文案》启动包` 的「钩子库」六型）：**结果量化**（30 秒出片 / 6 小时→60 分钟 / 40 分→72 分）· **对比反差**（使用前 ❌ vs 使用后 ✅，对比图版式③最好用）· **稀缺倒数**（最后 X 位 / 只到月底 / 限时价 RMxxx，具体数字放大字）· **身份认同**（做自己就是流量）· **痛点戳心** · **揭秘反直觉**。海报大字 = **一个主钩子 + 一个具体数字**，别塞满句子。
>
> 🔴 **真实性红线（海报上的字也守）**：**量化结果 ✅ · 稀缺倒数 ✅ · 具体数字 ✅ —— 但必须真、交付得出**。❌ 绝不写「月入过万 / 躺赚 / 不用努力 / 保证成功 / 全马第一 / 100% 有效」这种浮夸不实的话；❌ 假倒数（写「最后 10 位」其实随时能报）。判准：**这个数字/承诺客户来问、你敢当面兑现吗？** 敢＝放，心虚＝改。**卖结果，不卖幻觉。**（完整版见 02「§二·六 真实性红线」。）
...
| **评论 / 聊天截图卡** | 询问 / 见证社证（✅ 鼓励用）| `a realistic chat / comment / DM screenshot card, rounded, subtle shadow` 🔴 用**真**截图后期贴、或留占位；别让模型编假的冒充真客户 |
...
> 🔴 **假文字三形态（分开守，别混）：** ① **见证（✅ 鼓励用、只是要真）** —— 评论 / 聊天 / 案例截图是**很强的社证、鼓励做**，但要**真的**（真客户后期贴，或先留占位让用户填真的），**别让模型编假的冒充真客户发**。**见证本身不是红线。**② **乱码 UI** —— 画面里屏幕 / 图表 / 手机内容一律 `blurred abstract, NO legible text`（防乱码）。③ **白名单短标签** —— 只渲染**引号里指定的中文**，其余不出字（品牌 / 工具英文除外、要拼对）。
...
**🍱 实战配方速查（照高转化 AI 营销广告提炼 · 直接抄组合）**

> 每条 = **版式 × 风格 × 元素 × 尺寸**，挑一条当起点、再换成本品牌内容；同一批换着用别都一样。详细逐张拆解见《SUMA 广告拆解手册》。

| 配方 | 组合 | 何时用 |
|---|---|---|
| **前后对比分屏** | 版式③ × 风格B × [受众pill＋mega数字＋横幅＋对比分屏＋悬浮UI＋技能打勾排] × 3:4 | 痛点强、"用前 vs 用后"（SUMA 补习班款）|
| **单人导师权威** | 版式①/④ × 风格C × [换框大标题＋单人半身＋轮廓光＋CTA条] × 3:4 | 建立"跟对人"信任、卖分享会 |
| **超大数字对比** | 版式③ × 风格C/D × [对比mega数字＋闪电特效＋人物陪衬] × 3:4 | 量化收益（6小时→60分钟 / 90天 / 7个技能）|
| **学员合照社证** | 版式⑤ × 风格A × [周期mega＋成果句＋技能icon＋学员合照拼贴] × 1:1 | 招生、要信任背书（合照后期贴）|
| **信息密集双列** | 版式⑤ × 黑金 × [大标题＋价值横幅＋双列icon清单＋工具/认证logo带] × 4:5 | 讲清"到底教什么"（BoFu 理性受众）|
| **霓虹赛博** | 版式④ × 风格D × [周期数字＋游戏化词＋收入锚＋工具logo条＋霓虹全息] × 3:4 | 年轻/副业受众（D 慎用、紫要克制）|
...
| **团队合照品牌** | 版式⑤ × 品牌撞色 × [**团队统一服装合照**居中＋主张横幅＋升级/门面词] × 1:1 | 卖「有一整个团队/机构」（师资/团队门面）🔴 真团队照后期贴 |

## 🧭 出图提问清单（固定顺序 · 每张图都照这个问 · 教学统一）

> 🔴 **这是出海报/成品图的唯一提问脚本 —— 顺序写死、选项统一，因为要教学员，30 个人问出来必须一模一样。** 每题一张 `AskUserQuestion` 选项卡（≤4 个 + 「其他」，多选题标明），一题一题问、答完再问下一题。**每题都给「⭐ 推荐」（我按学到的搭配替他挑）或「自己定」两条路** —— 想省事就点推荐、想掌控就自定义。业务 md / branding 已记的（语言 / 受众 / 品牌色 / logo）→ **一句确认、不重问**；🔴 **例外：logo 用哪个版本 + 放哪个位置，出成品时必再问一次**（Cindy 定）。
>
> 🔴🔴 **Q1–Q13 每一题都要单独问、等学员答 —— 绝不自己「一并敲完」剩下的题！** 别出现「我敲定了 / 我一并敲的其余几题」+ 一张"定案表"（把配色/标题/元素/尺寸自己定好）。哪怕答案很明显、哪怕想省时间：**问完 Q5 就停下等他答，再问 Q6，再问 Q7…**，你只能标 ⭐推荐、决定权永远在学员（这是教学课，他要自己经历每一题）。



> 🎯 **定位铁律（出图前先锁死 · 最容易错的一条）：这张海报卖的是「你(user)自己的服务/产品」给「你的买家」—— 不是做成「你买家那个行业」的科普/推广。**
> 三层别搞混：**① 谁在卖 = user · ② 卖什么 = user 的服务/产品（＝海报主体、要突出的东西）· ③ 卖给谁 = 买家（＝海报受众）**。买家的行业/场景只当**背景 + 证据**，绝不当主题。
> 例：一个「**帮美业投 FB/IG 广告的代运营 agency**」——海报主体 = <b>它的投放服务 + 成效 dashboard 截图 + 做过的广告样本</b>，受众 = **美业老板**；🔴 **绝不做成一张美容/护肤科普图**（那是买家的行业、买家的买家，滑错层了）。
> 🔴 **出图前必做一句话确认**：「这张在卖 **[user 的服务/产品]** 给 **[买家]**，对吗？」—— 读业务 md 的"一句话生意"填进去；读不出或对不上就**先问清**，别闷头按买家行业出图。
> **服务型生意（agency / 自由职业 / 顾问 / 教练 / SUMA 自己）尤其容易滑错** —— 主体要放**你能交付的结果**（成效截图 / 案例 before→after / 作品样本 / 服务承诺 / 价格锚 / CTA），不是买家的产品。买家行业只用来"举例证明我做得到"。
>
> 🔴 **第二条（实测最常踩的坑）：「卖什么」不只写进文案，更要「长在画面上」。** 光在标题写"帮你管 social media"没用 —— 如果主视觉是「一个老师在书房看报表」，整张**看起来就像补习中心的广告**，不像 marketing agency。**主体 / 氛围 / 道具 = 你服务品类的信号**，让人一眼看出"这是做什么的服务"。
> **服务品类 → 画面信号速查（主视觉照这个放，🔴 别放"买家自己在做本行"的场景）：**
> - **Marketing / social media 代运营** → 社媒管理台 / 内容日历 / 手机里一排**成品贴文·Reels 封面** / 成长曲线 / **投放 dashboard（Results·Reach·Cost 带数字）** / 剪辑时间线 —— 画面要有"**在帮你运营内容**"的操作感（参 beauty_aesthetic_media：悬浮 dashboard + 手机展作品样本）。
> - **设计 / 拍摄** → 作品集拼贴 / 前后对比 / 相机·修图界面。
> - **顾问 / 教练** → 咨询场景 / 白板策略 / 成长路线图 + 客户成果数字。
> - **电商 / 实物产品** → 抠图产品 hero（版式②）。
> - **补习 / 才艺 / 美业…（🔴 只有当 user 本人就是这行时）** → 才放教学/学生/美容场景。
> 🔴 **一句话区分**：「开补习中心的你」= 用买家来定**称呼 pill / 痛点**（对的）；但**画面主体**必须是**你的服务**（社媒管理台 / 内容 / dashboard），**绝不是买家在教书 / 看书房**（错 —— 那看起来是补习中心的广告，不是你代运营服务的广告）。买家只当"被你服务的对象"出现，不当主角场景。
> ✅ **自检**：出图前问自己「**把标题文字遮住，光看画面，认得出这是卖什么服务的吗？**」认不出 = 主视觉滑到买家行业了，重挑主体。

**A. 通用三题（每个 skill 都先问，顺序固定）**
1. 🌐 **内容语言** —— ① 🇲🇾马来西亚华文（默认）② 规范中文 ③ English ④ BM ⑤ 中英 Rojak（第 5 放「其他」）。定图内文字语言（对话仍中文）。
2. 🎯 **做哪个受众** —— 🔴 **读业务 md，把 01 定好的受众逐个列成选项卡**（每个带一句 persona：谁 + 最痛的点），让他挑这条打哪个；**别只写「给几个常见」**。业务 md 没定受众 / persona → 先回 `01-branding` 把受众 + persona 卡定好再来（01 有分类菜单 + 7 项 persona 深挖）。选定后，这条的画面 / 语气 / 痛点全照那张 persona 卡走。
3. 🎚️ **哪个漏斗层** —— ToFu（拉新·只戳痛点不放价）/ MoFu（考虑）/ BoFu（成交·可放 offer 价）。

**B. 出图专属十题（固定顺序）**
4. 🧱 **主体是什么**（决定推荐版式）—— ① 人物/服务 ② 实物产品 ③ 信息/清单/对比 ④ 抽象/概念。
5. 🎨 **版式 / 配方** —— 🔴🔴 **把全部 13 套配方列成编号菜单给学员自己挑**（不是 Claude 砍成 3–4 个 —— 库是现成的，全列出来）。因超过 4 个 → **用文字编号菜单**呈现（不用 4 选卡），按他生意把最合适的 **1–2 套标「⭐推荐」放最前**，但 **13 套全列**，让他回数字：
   > 1. **前后对比分屏** — 挣扎↔自信（ToFu 戳痛点最稳）
   > 2. **单人导师权威** — 深色·建"跟对人"信任 / 卖分享会
   > 3. **超大数字对比** — 6h→60分（量化收益）
   > 4. **学员合照社证** — 招生 / BoFu 信任（合照后期贴）
   > 5. **信息密集双列** — 讲清"到底教什么"（BoFu 理性）
   > 6. **团队合照品牌** — 卖"有一整个团队 / 机构"
   > 7. **霓虹赛博** — 年轻 / 副业（风格 D 慎用、紫克制）
   > 8. **概念联名** — 要记忆点（换自有代言人 / 学姐AI）
   > 9. **服务成效证明** — 卖服务 / 代运营（dashboard + 作品样本）
   > 10. **Webinar 报名** — ToFu/MoFu 拉新报名
   > 11. **数字产品 / 课程** — BoFu 卖课 / 软件 / 模板（$27 款）
   > 12. **案例 + 价格** — BoFu 卖服务套餐
   > 13. **紧迫催单（合规版）** — BoFu 催单（🔴 名额有限、不写数字/免费）
   > ＋「14. 我自己描述版式」。🔴 选定配方后**版式骨架（①纪实/②抠图/③破框/④3D/⑤拼贴）自动带出**，不用他再选骨架。逐张精细拆解见 **《广告拆解手册》** `https://claude.ai/code/artifact/f58ad81c-fd22-44a4-a303-41ac3bf3b7ac`。**批量出多张换不同配方**。
6. 🖌️ **配色** —— ① **用品牌色**（读 branding 主色/辅色，默认）② 品牌色 + 1 个强调色 ③ 换其他配色（说要什么感觉：温暖/科技/高级…）④ 深色科技风。🔴 **branding 有品牌色 → 问「这张用品牌色还是其他？」；没记 → 先问品牌色是什么**。
7. 🔠 **标题设计**（分 4 小问，仍一题一张卡）——
   a) **标题文字**：自己打，或「帮我拟 3 个」选一个（开放题）。
   b) **字号层级**：① 大数字/大词 hero 型（一个词最大，如「1天」）② 大标题型（一句话最大）③ 均衡型。
   c) **字体感**：① 粗黑·醒目有力（默认）② 圆润·亲和 ③ 手写·随性 ④ 简约·干净。
   d) **正文字颜色**：① 白字+黑描边（最稳百搭·默认）② 品牌色 ③ 亮黄 ④ 其他。
8. 🧩 **主视觉元素（可多选）** —— 🔴🔴 **把整个 30 元素库分组列全给学员勾**（不是 Claude 挑几个），让他回编号多选；Claude 只按漏斗层在几个后面标「⭐」：
   > **文字 / 数字**：① 超大数字 hero ② 受众 pill ③ 色块横幅 ④ 单字变色高亮 ⑤ icon 打勾清单 ⑥ 四格利益条
   > **价格 / 紧迫（只 BoFu）**：⑦ 价格锚 ⑧ 星爆价格徽章 ⑨ 金色星爆 ⑩ 原价删除线对比 ⑪ 紧迫 ribbon（名额有限）
   > **证据 / 社证**：⑫ 学员合照拼贴 ⑬ 评论/聊天截图卡 ⑭ 数据成效面板 ⑮ 案例缩略图网格 ⑯ E1 作品环绕圈
   > **科技 / 对比 / UI**：⑰ 悬浮霓虹 UI ⑱ 前后对比分屏 ⑲ 前后双栏对照 ⑳ 手机+悬浮内容卡
   > **产品 / 工具 / 平台**：㉑ 产品/软件 mockup 拼贴 ㉒ 产品堆叠 ㉓ 工具 logo 徽章卡(斜45°) ㉔ 工具 logo 条 ㉕ 平台 icon 条
   > **其他**：㉖ 环绕 icon 徽章圈 ㉗ HRDC/认证徽章 ㉘ webinar 报名按钮 ㉙ E3 手指指向证据 ㉚ 学员合照(单张头像)
   > 🔴 真 logo / 合照 / 截图 / 认证一律**后期贴**，prompt 别画。

8b. 🕺 **人物动作 pose**（主体有人时才问）—— 🔴 **列全 A–I 让他挑**（Claude 标⭐推荐；POSE 铁律＝动词+受力对象）：
...
> 🔴🔴 **先把设计从 0 问清楚，再写 prompt！** 想好 idea（概念/角度）**不等于**可以动手写 prompt —— **必须先走完上面「🧭 出图提问清单」12 问**（尤其 **版式 / 配色 / 标题设计 / 元素 / 尺寸 / 用哪个 AI 工具**），把这张图的 **Design 一题一题问学员**、他答了、确认了，**才**填下面模板。🔴 **绝不跳过提问、绝不自己拍板直接套模板写 prompt**（这是彩排踩的坑：没问风格就直接出图）。
>
...
> **统一提问顺序（固定，别临时调换）：** 第一题 · 🌐内容语言 → 第二题 · 做哪个受众 → 第三题 · 哪个漏斗层（ToFu/MoFu/BoFu）→ 第四题起 · 本 skill 专属。内容语言 / 漏斗层若业务 md 已记 → 一句确认、不重复问。🔴 **受众这题：读业务 md 把已定的受众逐个列成卡（带 persona 一句话）让他挑，别只写「给几个常见」**；没定 → 回 01 用「分类菜单 + 7 项 persona」定好再来。选定后照那张 persona 卡走（痛点 / 语气 / 画面 / 本地人物）。
