...
5. 🎨 **版式 / 配方** —— 🔴🔴 **别只丢 5 个骨架**。从**完整配方库（「🍱 实战配方速查」12 套 + 「版式库」5 骨架）**里，按「主体 + 受众 + 漏斗层」用 CMO 判断**推荐 3–4 套最合适的具名配方**给他挑（每套 ＝ 已搭好的 版式×风格×元素，比裸骨架具体好用），推荐那套排第一标「⭐推荐」，末尾加「🔎 看全部配方」+「其他(自己描述)」。
   > **配方清单（挑着推）**：前后对比分屏 · 单人导师权威 · 超大数字对比 · 学员合照社证 · 信息密集双列 · 团队合照品牌 · 霓虹赛博 · 概念联名 · 服务成效证明 · Webinar报名 · 数字产品/课程 · 案例+价格 · 紧迫催单(名额有限)。
   > 🔴 选定配方后，**版式骨架（①纪实/②抠图/③破框/④3D/⑤拼贴）从配方自动带出**，不用他再单独选骨架。逐张精细拆解（标题/排版/特效/为什么有效）见配套 **《SUMA 广告拆解手册》** `https://claude.ai/code/artifact/f58ad81c-fd22-44a4-a303-41ac3bf3b7ac`。**批量出多张时换不同配方**，别整批同一个。
...
8. 🧩 **主视觉元素**（**可多选** · 给「⭐推荐组合」或自己勾）—— 大数字 hero / 受众 pill / 色块横幅 / 打勾清单 / 紧迫 ribbon（🔴用「名额有限」）/ 价格锚（BoFu才）/ 悬浮 UI / 学员合照（后期贴）/ 前后对比分屏 / 工具 logo 条（后期贴）。
