...
> 🔴 **进来先分流（三条路，看「手上有什么素材」定，不是看「想做什么」）：**
> - **B · 有原视频（脸＋声音都在）** → 换外壳：背景／动作／妆／衣服／发型／机位／运镜／背景活／虚化，**声音用原片的**（约 6.5 cr／秒，按原片长度计）
> - **② 只有声音档（mp3／mp4，没有脸）** 和 **③ 从零生成（有脚本）** → **同一条路（路线 A：数字人 ＋ 声音）**。
>   都要选数字人（真人样子／AI 人／卡通人），**唯一分叉在「声音怎么来」那一题**：② 用他现成的录音、**绝不重新配音**（写 `generate_audio:true` 会把他的录音丢掉）；③ 用 AI 声音或克隆他的声念台词。
>   🔴 技术设定照 SKILL.md「🎙️ 数字人 ＋ 声音 · 配对做法」那段的 **⚙️ 技术设定** 跑：`seedance_2_5` ＋ **`mode:"omni_reference"`**（漏了会变纯文字生成、照片和音轨被静默丢掉 —— 曾白烧 24 cr）· 480p 一支 ≈ 20 cr · `duration` 要跟配音等长。
>   🔴 **嘴型天生不保证准**（照片长出来的嘴），照那段的三层救援阶梯修 —— **前两层不花钱**。
>   💡 纯 AI 声音、不用锁脸的话，也可以走 `seedance_2_0` 480p+fast（一镜约 12 cr，最便宜；注意 `2_5` 没有 fast 模式）。
>
> 完整提问流程与铁律见 SKILL.md 的 **Step 0 · 分流** / **🎙️ 数字人 ＋ 声音 · 配对做法** / **🎭 路线 B** 三段。
