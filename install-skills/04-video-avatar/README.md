...
> - **C · 只有声音档（mp3／mp4 的声音，没有脸）** → **配数字人对嘴**：走 SKILL.md「🎙️ 路线 C ·「声音 ＋ 照片」配数字人」那段的 **C-2 入口**（声音已定稿，跳过克隆两步）。脸选真人样子／AI 人／卡通人。🔴 **技术设定照那段的「⚙️ 技术设定」跑**：`seedance_2_5` ＋ **`mode:"omni_reference"`**（漏了会变纯文字生成、照片和音轨被静默丢掉 —— 曾白烧 24 cr）· 480p 一支 ≈ 20 cr · duration 要跟配音等长。🔴 **绝不走路线 A** —— A 会自己生一把声，把他录好的声音丢掉。🔴 **嘴型天生不保证准**，照那段的三层救援阶梯修（先免费再花钱）
> - **A · 从零生成** → 有脚本，画面＋声音都由 AI 生（`seedance_2_0` 480p+fast 约 1.5 cr／秒，最便宜；注意 `2_5` 没有 fast 模式）
>
> 完整提问流程与铁律见 SKILL.md 的 Step 0 / 路线 A / 🎙️路线 C / 🎭路线 B 四段。
