...
- **开工只问两件（一题一张选项卡，别默认）：**
  1. **出镜的人是哪种？（三选一）**
     - **① AI 人**（AI 生成的虚拟主播，不是真人本尊）→ **再问一层（一张卡 · 二选一）**：
       - **a) Claude 自定义生成** — Claude 按你的**行业 + 目标受众**自己写 prompt，生成气质合适的 AI 人（快；不锁死一张脸，适合先 test 或不需要固定主播）。
       - **b) 九宫格选人锁定** — 出 9 张不同的脸让你挑一张、锁定，**全片同一张脸**（见 Step 3；适合要固定主播/人设）。
     - **② 你自己的真人样子** — 像你本人。**声音再问一层（一张卡 · 二选一）：**
       - **a) 生成马来西亚中文配音** — 直接用模型声（TTS）念台词，**不用录音**，最快；口音是标准华语底、靠 prompt 往本地味推，未必百分百「马来西亚腔」。
       - **b) 录一段自己的声音克隆** — 你录 ~1 分钟干净录音 → 克隆成你的声（见 Step 3），**真正是你本人的马来西亚口音**，最贴脸。
     - **③ 卡通人** — 风格化角色（2D 扁平 / 3D 皮克斯感 / 手绘 / 其他），生成一张角色图锁定再拍。
  2. **说话语言 —— 别默认英文：** 🇲🇾 **马来西亚华语／中文（默认）** · 中英混（Rojak）· English · Bahasa Malaysia ·（方言走「其他」）。选的语言 = 台词语言 + 选声语言，全片统一。
  3. **🔴 有没有参考？**（一张卡 · 三选一）
     - ① **有参考图**（想要的人物气质 / 场景 / 打光）→ 收下，🔴 **只借气质·场景·光，绝不照抄脸**（要固定脸走 Step 3 九宫格）。
     - ② **有对标片 / 想要的风格**（贴链接或说帐号）→ 你看完**用文字讲出你看到的几个点，跟他对一次**（「我看到的是：口播近景、暖色办公室、节奏很快 —— 你要的是这几样吗？」），确认后才写进每一镜 prompt。
     - ③ **没有参考，你帮我定** → 你按**业务 md 的行业 + 受众**推导（场景 / 穿衣 / 打光），给他 3–4 个选项点。
     > 🔴 参考只用来定**气质 / 场景 / 光 / 节奏**，**不定台词**（台词一律照 02-copy 的脚本走）。
- 确认「出镜的人 + 语言 + 参考 + 脚本齐」→ 才进下面的流水线。**怎么锁定这张脸（九宫格 / 克隆真人 / 卡通）见 Step 3。**
...
2. **想改什么？（第一层 · 只选一类）** —— 🔴 **一次只改一处**（见 B-3 铁律）：
 ① 妆 · ② 衣服 · ③ 发型 · ④ 背景/场景 · ⑤ 机位/角度 · ⑥ 其他

3. **🔴 有没有参考？（先问这题，再决定要不要开菜单）**
 > 一张卡、三选一：
...
6. **声音确认**：默认**用原片的声音**（不重新配音、不用 TTS）。

### 🔴 B-1.5 · 整支一次出？还是分段出？（问完素材马上问，决定 model 和钱）

**路线 B 跟路线 A 最大的不同：原片已经存在，所以不一定要分 shot。** 一张卡、二选一：

> **① 整支一次出（推荐 · 原片多长就出多长）** —— 原片就是时间轴，AI 照着重绘一遍，**声音、节奏、对嘴全部天然对齐**，不用拼接。
> **② 分段出** —— 只改其中一小段，或想每段用不同设定（例如前半换背景、后半换机位）。出来之后**要自己拼**。
