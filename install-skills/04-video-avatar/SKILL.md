...
description: >-
 ...
 Step 0 给 **三个入口**，但下游只有 **两套做法**：
 **① 有原视频（脸＋声音都在）→ 换外壳**（背景 / 动作 / 妆 / 衣服 / 发型 / 机位 / 运镜 / 背景活 / 虚化，**声音用原片的**）＝ 路线 B ·
 **② 只有声音档（mp3/mp4，没有脸）** 和 **③ 从零生成（有脚本）** ＝ **同一条路（路线 A：数字人 ＋ 声音）**，
 都要选数字人（真人样子 / AI 人 / 卡通人），**唯一差别在「声音怎么来」那一题**：② 用他现成的录音、绝不重配；③ 用 AI 声音或克隆他的声念台词。
 **由第一题决定走哪条**，之后一题一题问到底。
 跟用户对话一律用中文；教练式一题一题问。
metadata:
 type: reference
...
> 　· **出来是**：一个人在讲你那段录音，嘴型对上
> 　· **大概花**：一支约 **20 credits**（480p · 照片＋声音那套的实测价）
> 　· **走**：**跟 ③ 同一条路（路线 A）** —— 差别只有「声音怎么来」那一题：你的声音已经有了，直接用
> 　· ⚠️ **嘴型天生不保证一次准**（照片长出来的嘴），照「🔧 嘴型对不上怎么救」的三层阶梯修，**前两层不花钱**
>
> **③ 从零生成一支新的片**
> 　· **你手上有**：一份完整脚本（没有的话我先带你回 `02 写文案` 写好）
> 　· **没有的是**：画面和声音，两样都由 AI 生
> 　· **人可以是**：你的真人样子（3 张照片锁脸）／AI 人／卡通人 —— 等下问你
> 　· **声音**：**AI 声音念台词** 或 **录 1 分钟克隆你自己的声来念** —— 等下问你
> 　· **跟 ② 的差别只有一格**：② 的声音已经有了，③ 的声音要生成。**选人、锁脸、生成画面、对嘴、修嘴型全部一样**
> 　· **出来是**：一支 8 镜左右、约 60–100 秒的完整口播片
> 　· **大概花**：**最便宜** —— 纯 AI 声音走 `seedance_2_0` 480p+fast，一镜约 **12 credits**；要锁你自己的脸（照片＋声音那套）则一支约 **20 credits**
> 　· **走**：路线 A
>
> **④ 我只有照片 / 我不确定**
...
- 选 **②** 或 **③** 或 **④（只有照片）** → **同一条路（路线 A）**，接着跑下面的 **Step 1–8**。**唯一的分叉在 `A-Q1b`「声音怎么来」那一题**：
  - 选 **②（只有声音档）** → `A-Q1b` 直接锁定 **③ 用现成录音**（先量四件事：总长／哪几秒在讲话／语言口音／干不干净），**跳过做声音那两步**，从生成画面开始。🔴 **绝不 `generate_audio:true`** —— 那会自己生一把声，把他录好的那段丢掉。
  - 选 **③（从零生成）** → `A-Q1b` 正常问他要 **① AI 声音** 还是 **② 克隆自己的声**。
  - 选 **④（只有照片）** → 照 ③ 走，多收 1–3 张照片当锁脸参考。
  - 🔴 三者的画面做法完全一样，技术设定一律照「🎙️ 数字人 ＋ 声音 · 配对做法」段的 **⚙️ 技术设定** 跑，别自己改（`mode:"omni_reference"` 漏了会白烧 credit）。
- 选 **④** → **反问这两题**（照抄，一题一张卡）：
...
**A-Q1b · 声音怎么来？（照抄 · 🔴 这题是 Step 0 的 ② 和 ③ 唯一的分叉点，其余步骤完全一样）**

> 🔴🔴 **想清楚再问：Step 0 的 ②（只有声音档）和 ③（从零生成）不是两条路线，是同一条路线的两个入口** —— 差别**只有这一格「声音从哪来」**。选完人、锁完脸、生成画面、对嘴、修嘴型、三件套…… **下游一模一样**。（2026-08-24 修：别再为「只有声音档」另开一套流程。）

> **这支片的声音怎么来？（点一个）**
> **① AI 声音念台词** —— 用 AI 的声念你的台词，**不用录音**，最快。口音是标准华语底、靠 prompt 往本地推，**未必百分百马来西亚腔**。
> **② 录 1 分钟，克隆你自己的声来念台词** —— 你录一段约 1 分钟的干净录音，克隆成你的声，再让它念台词。**真正是你本人的口音**，最贴脸。
> **③ 我已经有现成录音了（mp3／mp4）** —— 直接用那段，**不重新配音**（口音、语气、停顿全是原本那个人的）。**台词就在录音里**，所以不用另外写脚本；没有逐字稿的话我帮你听打再给你核对。

- 选 **①②** → 照下面「🎙️ 数字人 ＋ 声音」那段的 **步骤 1️⃣ 先做声音**（先把声音搞定才动画面）。
- 选 **③** → **跳过 1️⃣ 2️⃣，声音已经定稿**，直接从 **3️⃣ 生成画面** 开始。但要先免费量四件事：**总长几秒 · 哪几秒在讲话（`silencedetect`，决定切镜点）· 什么语言口音（写进 prompt 的 `Spoken in …`）· 干不干净（有背景乐/回音会让对嘴飘）** —— 量完把数字讲给他听。
- 🔴 **三个选项之后都一样**：选人锁脸 → 场景穿衣 → 真实感三件套（运镜/背景活/虚化）→ 生成 → 对嘴修正阶梯。
...
## 🎙️ 数字人 ＋ 声音 · 配对做法（Step 0 的 ② 和 ③ 都走这一段 —— 差别只在「声音从哪来」）

> 这是「你的真人样子 ＋ 你自己的克隆声」这个组合的完整做法。**不用出镜、不用化妆、可以批量做** —— 代价是嘴型一定要后期修（见下面）。

> 🔴🔴 **三种声音来源，同一条流程（2026-08-24 补）** —— 差别只在开头那一格，**从「生成画面」开始完全一样**：
> | 声音来源 | 从哪一步开始 |
> |---|---|
> | **① AI 声音念台词** | 步骤 1️⃣（先做声音）|
> | **② 克隆你自己的声念台词** | 步骤 1️⃣（先录 1 分钟 → 克隆 → 念台词）|
> | **③ 他已经有现成录音**（Step 0 选 ②）| **跳过 1️⃣ 2️⃣**，声音已定稿 → 直接 **3️⃣ 生成画面**。那段录音**就是**要对嘴的音轨，**绝不重新配音**（口音／语气／停顿就是资产，重配就毁了）|
>
> **来源 ③ 拿到声音档，先免费量这四件事，再谈别的（量完把数字讲给他听，别只说「收到了」）：**
> ① **总长几秒** → 决定要切几镜、总共多少钱 · ② **哪几秒在讲话、哪几秒静音**（`silencedetect`）→ 决定镜头切点 · ③ **什么语言／口音** → 写进每镜 prompt 的 `Spoken in …`，嘴型才对得上 · ④ **干不干净**（有背景乐／回音 → 对嘴会飘，先问他有没有干净版）
>
> **来源 ③ 还要多问两题（一题一张卡）：**
> 1. **有没有逐字稿？** ① 有，我给你（嘴型最准）· ② 没有，你帮我听打（我转录后给他核对，人名品牌名机器常听错）· ③ 不用稿直接对嘴（会差一点）
> 2. **画面要几个镜头？** ① 一镜到底（≤15 秒才建议）· ② 切成几镜：人像 ＋ B-roll 交替，**声音不断、画面轮换**（15 秒以上一律这个）· ③ **全部 B-roll 不出人脸**（🟢 **完全没有对嘴问题、最稳最省事** —— 声音是旁白而不是「我」在讲的话，优先推这个）· ④ 你帮我判
>  🔴 判断规则（他选 ④ 时照这个）：≤15 秒 → ① · 15–60 秒 → ② · >60 秒 → ② 且一定要有 B-roll 分担 · 旁白型 → ③
>  🔴 **切镜一律切在句子之间**（用 ②「哪几秒在讲话」抓的静音点），绝不切在一个字中间；**成片后用原始声音档整条铺回去**，不要把切开的小段接起来（接口会有细微断点）。
>
> 🔴 **「真实感三件套」（运镜 · 背景要不要活 · 背景要不要虚化）这条路同样要问** —— 画面是从零造的，比重绘更容易「一眼假」。库和 prompt 句子见路线 B 的第 5/6/7 题。
