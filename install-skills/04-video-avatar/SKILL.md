 ...
 **C 只有声音档（mp3/mp4 的声音，没有脸）→ 配数字人对嘴**（＝「声音＋照片」那条已测路线的 C-2 入口：真人样子 / AI 人 / 卡通人，**声音用他那段录音、绝不重配**）·
...
- 选 **②** → 走 **路线 C**，**跳到本文件后面的「🎙️ 路线 C ·「声音 ＋ 照片」配数字人」段，照入口 C-2 走**（声音已定稿，跳过克隆那两步，直接生成画面）。🔴 **别当成路线 A** —— 路线 A 会 `generate_audio:true` 自己生一把声，把他录好的那段直接丢掉。🔴 **技术设定照那一段的「⚙️ 技术设定」跑，别自己改**（`mode:"omni_reference"` 漏了会白烧 credit）。
...
## 🎙️ 路线 C ·「声音 ＋ 照片」配数字人（＝ Step 0 选 ② 走这里 · 也是 A 路最常用的一种组合）

> 这是「你的真人样子 ＋ 你自己的克隆声」这个组合的完整做法。**不用出镜、不用化妆、可以批量做** —— 代价是嘴型一定要后期修（见下面）。

> 🔴🔴 **两种入口，同一条路（2026-08-24 补）：**
> - **C-1 他要现场克隆自己的声** → 照下面「步骤 1️⃣ 先做声音」走（录约 1 分钟 → 克隆 → 念台词）。
> - **C-2 他已经有一段现成录音**（Step 0 选 ②「我只有声音档，没有人脸」）→ **跳过步骤 1️⃣ 和 2️⃣，声音已经定稿了**，直接从 **3️⃣ 生成画面** 开始。他那段录音**就是**要对嘴的音轨，**绝不重新配音**（他的口音／语气／停顿就是资产，重配就毁了）。
>
> **C-2 拿到声音档，先免费量这四件事，再谈别的（量完把数字讲给他听，别只说「收到了」）：**
