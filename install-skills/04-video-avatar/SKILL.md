...
description: >-
 SUMA AI 内容系统 · 第 04 步。把脚本做成完整口播短视频（含数字人：选AI人/真人样子/卡通人 + 生成 + ChatCut 剪辑）。
...
9. **背景音乐 BGM？** → ① 要，帮我做一首（接 `06-music`）· ② 我已有一首（给文件）· ③ 不要；选①/②再问一句「什么感觉：上进 / 温暖 / 都市 / 轻松」
10. **整体节奏？** → ① 快剪带感（默认，短视频）· ② 正常口播节奏 · ③ 慢稳
11. **片头片尾？** → ① 片尾放 logo + 一句 CTA 卡（默认）· ② 只片尾 logo · ③ 都不要

> 比例默认 **9:16 竖版**（不用问）。答完这 11 题 → 记进 Notion 立项页「剪辑需求」→ 才照 `CHATCUT-SKILL` 手册开剪。**没问就剪 = 违规重来。**
...
## 🎬 做口播视频 · 照 Step 1–8 走（一步做完再下一步）

### Step 1 · 开工前硬闸门（先有完整脚本才做口播视频）
