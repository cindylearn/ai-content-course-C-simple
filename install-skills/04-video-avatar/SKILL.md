...
- **开工只问两件（一题一张选项卡，别默认）：**
  1. **出镜的人是哪种？（三选一）**
     ...
     - **② 你自己的真人样子** — 像你本人 + 用你的声音（要 3 样素材，见 Step 3）。
...
**② 你自己的真人样子 —— 给 3 样素材（克隆脸 + 声）**
1. **照片** 3+ 张清晰正脸/多角度（正脸·侧一点·微笑）→ 锁脸。
2. **语音样本（关键）** ~1 分钟干净录音（安静·近麦·无背景乐）→ `create_voice_from_confirmed_audio` 克隆声；样本脏 → 克隆带杂音。
3. **可选·一小段说话视频** → 更像本人。
→ 出视频「声音分开」：静音生成 + 逐句 TTS 克隆声 + 按 PTS 合声对口型（唯一要拆的情况）。
