...
```
【本 skill】script → 拿脸参考图(真人 or Step 3 锁的数字人) → 逐镜 prompt → 生成
 ↓
...
> **一题一题问 · 一题一张可点选项卡 · 绝不一次全倒**（Claude Code 的 `AskUserQuestion` 交互组件）——问一题、等他答完、才问下一题。语言、场景、镜头方向… 让用户**点选**，不要只列 `1. 2. 3.` 让他打字。只有开放题（台词内容、具体挑刺）才让他打字。**每题选项卡最多 3–4 个**（硬限制，系统自动加「其他」）。**选人见 Step 3**：性别/年龄/气质/选几号脸都在 Step 3（锁定出镜的人），04 只用锁好的脸参考图。
...
> **先 test 一支正常口播就行 —— 直接照脚本 persona 出主播，画面+声音一起，不用先走 Step 3 锁人。**
> 想**全片同一个人 / 真人本尊 / 锁定一个特定 AI 脸** → 才去 Step 3（锁定出镜的人）（选人在 Step 3）。

**Q1. 角色 / Character**（一题一张卡 · 三选一）
1. **AI 人**（default）→ 跟真人一样的照片级真人数字人，照脚本 persona 出
2. **你自己的真人样子** → 像你本人；声音再选 a) 生成马来西亚中文配音（不用录音）/ b) 录 ~1 分钟自己的声音克隆（最贴脸）
3. **卡通人** → 选一个卡通风格分类（2D 扁平 / 3D 皮克斯感 / 动漫 / Q版…）

**Q2. 说话语言 / Language**（别默认英文）
1. 🇲🇾 马来西亚华语/中文（默认）· 2. 中英混 Rojak · 3. English · 4. Bahasa Malaysia
> 选的语言 = 台词语言 + 选声语言，全片统一。

（有 Step 3 锁好的脸参考图？→ 一起用锁人；没有 → 照脚本 persona 直接出，先 test。）
...
- 主角脸 Face: [用锁定的脸参考图 —— 真人 or Step 3 锁的数字人，不重选]
- 场景 Scene: [具体地点 + 细节]
- 语言 Language: 🇲🇾 马来西亚华语/中文（默认）· 中英混 · English · BM
- 固定 Fixed: 9:16 · 照片级真实（真相机感，无 AI 味）
```
> 这样对吗？回复 **yes** 我就进逐镜 prompt。（没脸参考图 → 先去 Step 3（锁定出镜的人） 做一个、拿 3 张脸回来）
...
**目的：全片同一个人。这一步只是把「3 张脸参考图」备好 —— 选人/造脸是 Step 3（锁定出镜的人） 的活，04 不做。**

- **有真人本尊出镜**（老板/讲师）→ 直接用他的 **3 张脸参考图**（正脸 + 不同角度更稳）。
- **没有真人** → 去 Step 3（锁定出镜的人）：九宫格选人、锁脸、选 model 都在 Step 3；做好后拿回 **3 张脸参考图**。
