...
description: >-
 ...
 🔴 **方向不靠触发词猜，靠进来后问；而且判断标准是「你手上有什么素材」，不是「你想做什么」。**
 进来第一题（Step 0 分流）先确认这支片怎么做 —— **三条路**：
 **B 有原视频（脸+声音都在）→ 换外壳**（背景 / 动作 / 妆 / 衣服 / 发型 / 机位 / 运镜，**声音用原片的**）·
 **C 只有声音档（mp3/mp4 的声音，没有脸）→ 配数字人对嘴**（真人样子 / AI 人 / 卡通人，**声音用他那段录音、绝不重配**）·
 **A 从零生成**（有脚本，画面和声音都由 AI 生；数字人选 AI 人 / 真人样子 / 卡通人）。
 三条路都在这个 skill 里，**由第一题决定走哪条**。
 跟用户对话一律用中文；教练式一题一题问。
metadata:
 type: reference
...
> 4.5 🔴🔴 **「问」和「跑」是两件事，别把省钱铁律套到提问上。** 问问题**零成本**（问十样也不花一分），花钱的只有**生成**。所以：**需求一次问全、问到他说「没有了」为止**；省钱的分轮只发生在「跑」那一步。**绝不因为「一次只改一处」就连问都只问一样** —— 那是把学员的需求挤掉，他会觉得你在拒绝他。（2026-08-24 修，旧版真的有这个 bug）
> 4.6 🔴🔴🔴 **每一张卡都要「一眼就能选」——模棱两可 ＝ 这张卡作废，重写。**（2026-08-24 补，学员原话：「问题和选项要清晰，不能模棱两可」）
>
> **六条硬标准，出卡前逐条自检：**
> 1. **一张卡只问一个维度。** 🔴 最常犯：把「**确认**」和「**追加改动**」混在同一张（①对，只换背景 / ②光线也跟着变 / ③有一项要改）—— ① 是确认、②③ 是新需求，单选题会逼他一次选两个（实测他就真的答了「2，3」）。**确认归确认卡、追加归追加卡。**
> 2. **编号必须跟库的编号一致，且不准跳号。** 库里有 10 个却只塞 4 个进卡、号码变成 ③②⑧⑥ —— 看起来像乱给的。**做法：库 ≤4 个 → 用选项卡；库 >4 个 → 用文字编号菜单把「全部」列出来，让他打号码**（这就是铁律 4，别为了用卡而砍库）。
> 3. **每个选项写三件事：它是什么 · 出来会长什么样 · 风险／代价。** 只写形容词（「自然一点」「比较有质感」「适当」）＝ 废选项，他没办法比较。
> 4. **选项之间必须互斥**，不能有「②包含③」这种重叠；**可多选就在题目里明写「（可多选）」**，别让他猜。
> 5. **禁止模糊词**：自然一点／比较好／适当／优化一下／看情况。**改成可验证的说法**：「背景稍软，场景还看得清楚」＞「虚化自然一点」。
> 6. **选项要覆盖他真正的意图 —— 覆盖不了就先问上游那题。** 🔴 实测教训：问「换什么动作」给了手势／点头／指身后／边走边讲，他答的是「**站盲盒展示台前手拿实物**」（完全在选项外）—— 因为**没先问「这支片要展示什么东西」**。**选项一直被答在框外 ＝ 你漏了上游那题，回去补问，别硬套。**
>
> 🔴 **自检一句话**：把卡念给一个没参与过这个项目的人听 —— **他能不能不反问就选？** 不能，就是模棱两可，重写。
...
> 🔴 **model 不写死 —— 每次出片前先弹选项卡问学员用哪个**（Seedance 2.5 像真人 / Wan 2.7 嘴型准 / Seedance 2.0 fast 最省），**每个选项后面带 tips**；详见「🎬 出片前必问：这支用哪个 model」。省 credit 的规则不变：**一律用该 model 的最低画质档，先出 1 支满意才批量，真跑前 `get_cost` 预检。**
...
- 🔴 **model 不写死 —— 每次出片前先弹选项卡问学员用哪个**（每个选项带 tips），详见「🎬 出片前必问：这支用哪个 model」。**省 credit：一律用该 model 的最低画质档，先出 Shot 1 满意才批量，真跑前 `get_cost` 预检。**
...
> 🔴 **判断标准只有一个：你手上现在有什么？**（不是「你想做什么」——先看素材，路就定了）
>
> **你这支片要怎么做？（点一个就好）**
>
> **① 我有原视频：脸和声音都在，想换掉外壳**
> 　· **你手上有**：一支已经拍好的片（人脸 ＋ 说话声都在里面）
> 　· **能换什么**：背景／场景 · 动作 · 妆 · 衣服 · 发型 · 机位角度 · 运镜 · 背景要不要活 · 虚化
> 　· **声音**：**用原片的原声，不重新配音**
> 　· **脸**：还是他本人（原片那张脸）
> 　· **出来是**：同一个人、同一段话，但换了外壳
> 　· **大概花**：**贵** —— 按原片长度算，约 6.5 credits／秒（7 秒 ≈ 50 cr · 20 秒 ≈ 130 cr）
> 　· **走**：路线 B
>
> **② 我只有声音档（mp3／mp4 的声音），没有人脸**
> 　· **你手上有**：一段录好的旁白／口播录音（或一支只要声音、画面不能用的片）
> 　· **没有的是**：出镜的人 —— 所以要**配一个数字人来对嘴**
> 　· **人可以是**：你自己的真人样子（给我 3 张照片锁脸）／AI 人（虚拟主播）／卡通人 —— 等下问你
> 　· **声音**：**用你这段声音档**，不重新配音（口音、语气、停顿全是原本那个人的）
> 　· **出来是**：一个人在讲你那段录音，嘴型对上
> 　· **大概花**：中等（按每一镜的秒数算）
> 　· **走**：路线 C
>
> **③ 从零生成一支新的片**
> 　· **你手上有**：一份完整脚本（没有的话我先带你回 `02 写文案` 写好）
> 　· **没有的是**：画面和声音，两样都由 AI 生
> 　· **人可以是**：AI 人／你的真人样子／卡通人 —— 等下问你
> 　· **声音**：AI 生成配音（或你另外录 1 分钟克隆自己的声）
> 　· **出来是**：一支 8 镜左右、约 60–100 秒的完整口播片
> 　· **大概花**：**最便宜**（480p+fast，一镜约 12 credits）
> 　· **走**：路线 A
>
> **④ 我只有照片 / 我不确定**
> 　· **只有照片** → 走 ③，你的照片当人物参考让 AI 人长得像你（还是要脚本）
> 　· **不确定** → 我问你两题就知道该走哪条

**收到答案后怎么走：**

- 选 **①** → 走 **路线 B · 重绘已录好的片**，**直接跳到本文件后面的「🎭 路线 B」段**，别跑 Step 1–3（那是给从零生成用的，会叫他交脚本、锁三张脸，全错）。
- 选 **②** → 走 **路线 C · 声音档配数字人对嘴**，**跳到本文件后面的「🎙️ 路线 C」段**。🔴 **别当成路线 A** —— 路线 A 会重新生成配音，把他录好的那段声音丢掉。
- 选 **③** 或 **④（只有照片）** → 走 **路线 A**，接着跑下面的 **Step 1–8**。（只有照片的话，多收他 1–3 张照片当人物参考，其余照 A 走。）
- 选 **④** → **反问这两题**（照抄，一题一张卡）：
...
**Step 0-B · 报一句方向（🔴 不是回读确认，别在这里问 credit）**

分流答完，**用一句话告诉他你判到哪条路就好**，不用发卡、不用等他答：

> 「好，那我们走**〔路线 A 从零生成／路线 B 重绘原片〕**。接下来我一题一题问你。」

- 🔴 **这里绝不要回读「多长 · 几 credit」** —— 那时候你还没拿到原片／脚本，**讲不出真数字，只能瞎编**。
- 🔴 **真正的回读确认在最后**（见下方「🔴🔴 总回读闸门」）—— **全部题目答完、拿到真实数字之后才做**。

---

### 🔴🔴🔴 总回读闸门（全部问完 → 回读确认 → 才动手写 prompt）

> **这是本 skill 最重要的一道闸门。** 顺序写死，不准调换：
>
> ```
> 全部题目问完（一题一张卡）
>   → 🔴 总回读：把所有答案 + 真实 credit 数字，列成一张表给他对
>   → 他答「对」
>   → 才开始写 prompt
>   → prompt 写进 Notion
>   → 停下叫他去 check
...
> ```
>
> 🔴 **绝不**：还有题没问完就先写 prompt · 边问边写 · 「我先草拟一版你看看」 —— **prompt 一旦先出来，后面的题就会被你自己的草稿带着走**，学员也会以为已经定案了。
> 🔴 **绝不**：跳过总回读直接写进 Notion。
...
一张卡、二选一：**① 对，照这个做** · **② 有一项要改**（→ 他讲哪一项，只重问那一题，改完再回读一次）。

🔴 **预估花费一定要是真数字**（秒数 × 该 model 每秒价），不准写「不多」「大概几十」。拿不准就先 `get_cost:true` 免费预检。
...
## 🎙️ 路线 C · 只有声音档 → 配数字人对嘴（2026-08-24 新增）

> **什么时候走这条**：学员在 Step 0 选了 **②「我只有声音档，没有人脸」**。
> **跟另外两条的根本差别**：
> - **声音已经存在、绝不重新生成**（他的口音／语气／停顿是资产，重配就毁了）→ 所以 `generate_audio: false` ＋ 声音档当 `audio_references` 喂进去。
> - **脸不存在，要造一个**（真人样子／AI 人／卡通人）→ 要锁脸（跟路线 A 的 Step 3 共用同一套做法）。
> 🔴 **绝不当成路线 A 处理** —— 路线 A 会 `generate_audio: true` 自己生一把声，把他录好的那段直接丢掉。这是这条路最容易犯的错。
> 🔴 **也绝不当成路线 B** —— 路线 B 要有原片的脸可以照着重绘，这里没有。

### C-1 · 收素材（一题一题问）

1. **声音档** —— mp3 / m4a / wav / 或一支「只要声音」的 mp4。
 🔴 **拿到先量四件事再谈别的**（免费、不花 credit）：
 - **总长几秒**（决定要切几镜、总共多少钱）
 - **哪几秒在讲话、哪几秒是静音**（`silencedetect` 抓句子边界）
 - **讲什么语言 / 什么口音**（写进每一镜 prompt 的 `Spoken in …`，让嘴型和语言对得上）
 - **干不干净**（有没有背景乐／回音 —— 脏的音轨对嘴会飘）
 量完把数字讲给他听，别只说「收到了」。

2. **有没有对应的脚本／逐字稿？（照抄）**
 > **这段声音的文字稿你有吗？（点一个）**
 > **① 有，我给你** —— 最好。每一镜的 prompt 会带上那一句台词，嘴型最准
 > **② 没有，你帮我听打** —— 我用转录抓出来，再给你核对一次（人名／品牌名机器常听错）
 > **③ 不用文字稿，直接对嘴就好** —— 也行，但嘴型会比有稿差一点

3. **谁出镜？（照抄 · 跟路线 A 同一个库）**
 > **这段声音要配谁的脸？（点一个）**
 > **① 我自己的真人样子** —— 给我 1–3 张清晰正脸照，出来长得像你本人（最贴，因为声音本来就是你的）
 > **② AI 人（虚拟主播）** —— 不用露脸。要固定同一张脸的话我再出九宫格给你挑
 > **③ 卡通人** —— 风格化角色，不像真人

4. **一镜到底，还是切成几镜？（照抄 · 🔴 这题决定成品好不好看，也决定钱）**
 > **画面要几个镜头？（点一个）**
 > **① 一镜到底** —— 从头到尾同一个人对着镜头讲完。最省事；但超过 15 秒会有点闷，而且**单镜有长度上限**（2.5 最长 30 秒）
 > **② 切成几镜（人像 ＋ B-roll 交替）** —— ⭐ 声音不断，画面轮着换：人讲话 → 相关空镜 → 再回到人。**长录音只能这样做**，也比较耐看
 > **③ 全部 B-roll，不出人脸** —— 只有旁白 ＋ 画面（等于纪录片旁白）。**完全没有对嘴问题、最省事最稳**，适合声音是旁白不是自我介绍
 > **④ 你帮我判** —— 我按你的录音长度和内容定

 🔴 **判断规则（他选 ④ 时照这个）**：≤15 秒 → ①；15–60 秒 → ②（每镜 5–10 秒）；>60 秒 → ② 且**一定要有 B-roll 分担**；声音是旁白不是「我」在讲 → ③。

5. **然后走「真实感三件套」那三题**（第 5/6/7 题：运镜 · 背景要不要活 · 背景要不要虚化）—— 🔴 **路线 C 同样要问**，从零造出来的画面更容易「一眼假」，这三题只会更重要。

6. **场景 / 穿衣** —— 照路线 A 的做法问（指定场景 or 让 AI 按服务品类构思；指定服装 or 按人物气质定）。

### C-2 · 怎么切镜（🔴 按声音切，不是按秒数硬切）

1. 用 `silencedetect` 抓出**句子边界**（静音处），**镜头切点一律落在句子之间**，绝不切在一个字中间。
2. 每一镜的 `duration` ＝ **那一句讲话的实际长度**（🔴 见下面 C-3 铁律 ②）。
3. 一镜的声音怎么给：把声音档**按句切成小段**，每一镜喂它自己那一段当 `audio_references`。
4. **成片后用原始声音档整条铺回去**（不是把切开的小段接起来 —— 接口会有细微断点）。这样声音是完整连续的，画面是切开的。

### C-3 · 🔴🔴 对嘴五条铁律（跟路线 A 的口播镜共用，这里再点一次因为最容易漏）

1. **`generate_audio: false`** ＋ 声音档进 `audio_references`。🔴 写成 `true` ＝ 他的录音白给了。
2. **`duration` 不能长于那一句的讲话长度** —— 长了模型会「把嘴型摊开」填满整段，慢半拍。宁可切掉尾巴静音。
3. **每一镜的配音不要短于约 5 秒** —— 实测 3.72 秒的镜对嘴失败、5.69 秒正常。**短句要合并，别为了「短好对嘴」硬拆**（反直觉但实测两次都如此）。
4. **LIP-SYNC 那句放 prompt 第一行**，并写明其他指令都次于把嘴做对（路线 C 的脸是新造的，身份没有「原片」可对照，所以这里**可以**把对嘴设为最高优先级 —— 跟路线 B 相反）。
5. **绝不写「嘴持续张开／全程不间断说话」**，也**绝不写头部大动作**（摇头／点头／大转头会跟嘴部动画抢资源，实测当场崩）。

### C-4 · 成本（按秒 × 镜数）

| 情况 | 怎么算 |
|---|---|
| 480p + fast（路线 A 那套省钱档） | **约 1.5 credits／秒** → 8 秒镜 ≈ 12 cr |
| 要锁脸／要更稳（seedance 2.5） | 约 6.5 credits／秒 |

🔴 **先算总数再报给他**：总秒数 ÷ 每镜秒数 ＝ 镜数；镜数 × 每镜秒数 × 单价 ＝ 总价。
🔴 **一律先跑第 1 镜给他看**（对嘴准不准、脸像不像、有没有烤字幕，全在这一镜挡掉），满意才跑其余。
🔴 跑之前用 `get_cost: true` 免费预检，把真实数字讲给他听、等他说「跑吧」。

### C-5 · 收尾（跟另外两条合流）

- 成片后**把原始声音档整条铺回去**（生成的是静音或分段声）。
- 字幕／花字／音效／BGM → 交给 `05-edit`（ChatCut）。
...
2. **想改什么？（第一层 · 照抄 · 🔴 做成可点的卡，别丢文字菜单让他打字）**
 ...
 > 🔴🔴🔴 **先把这条搞清楚，否则你会犯旧版的错：「问」和「跑」是两件事。**
 > **问 ＝ 免费**（问十样也是零 credit）· **跑 ＝ 花钱**（跑几轮才是钱）。
 > 所以：**需求要一次问全、问到他说「没有了」为止；分轮只发生在「跑」那一步。**
 > 🔴 **绝不**因为「一次只改一处」这条省钱铁律，就**连问都只问一样** —— 那是把学员的需求挤掉，他会觉得你在拒绝他。（2026-08-24 修：旧版就是这个 bug。）

 > **这支片你想改哪一样？（点一个）**
 > **① 造型（妆／衣服／发型）** —— 人本身的样子。点了我再问你是三样里的哪一样
 > **② 背景 / 场景** —— 换到别的地方拍（🔴 **这一项 AI 做得最好，把握度最高**）
 > **③ 机位 / 角度** —— 同一段话，换成从别的角度看（像多机位拍的）
 > **④ 全部都要改 / 我要改好几样** —— 没问题，我**一样一样问你**，问完再一起算钱怎么跑最划算
 > **⑤ 其他** —— 你打字讲

 **🔴🔴 2a · 选了 ④（或他自己回「1+2+3」「全部都要」）→ 进「清点循环」，一样一样问到他说没有了**

 顺序照这张表走，**一样一张卡、答完才问下一样**；他没提到的那几样也要问一次（他常常是忘了，不是不要）：

 | 顺序 | 问哪一样 | 细节菜单在哪 |
 |---|---|---|
 | 1 | 妆 | 2b → ① 的妆容库（6 种） |
 | 2 | 衣服（含颜色） | 2b → ② 的穿什么库（6 种）＋ 颜色 |
 | 3 | 发型／发色 | 2b → ③ 的头发库（6 种） |
 | 4 | 背景 / 场景 | 第 4 题 → ④ 换到哪（6 种） |
 | 5 | 机位 / 角度 | 第 4 题 → ⑤ 换成哪个（5 种） |
 | 6 | **运镜** | 第 5 题（10 种） |
 | 7 | **背景要不要活** | 第 6 题（可多选） |
 | 8 | **背景要不要虚化** | 第 7 题（5 种） |

 **每问完一样，紧接着问这一句（照抄 · 直到他选「没有了」才停）：**

 > **还有什么要改？（点一个）**
 > **① 没有了，就这些** —— 我把清单回读给你，然后算钱
 > **② 还有一样：〔下一个还没问的项目〕** —— 继续问
 > **③ 我要跳过剩下的** —— 剩下几样全部保持原片不变
 > **④ 其他（我打字讲）** —— 上面清单里没有的东西

 🔴 **循环出口只有两个**：他选 **①「没有了」** 或 **③「跳过剩下的」**。**没拿到这两个之一，就继续问下一样，别自己收尾。**
 🔴 **每一轮循环都要把「已确认的清单」摆出来**（一行一样），他才知道自己讲过什么、还剩什么没讲。
 🔴 **清点完 → 先回读完整清单 → 才进「怎么排」那题（2c）算钱分轮。**

 > 💡 **为什么这样才对**：改五样和改一样，**问的成本一样（零）**，但**跑的成本差五倍**。先问全，才有资格帮他判「哪几样可以塞进同一轮、哪几样必须分开」—— 只问一样的话，你根本不知道他后面还想干什么，等他讲出来又要重跑。
 ...
 **🔴🔴 2c · 清点完了 → 才算钱、才决定分几轮跑（必读 · 别临场自己发挥）**

 > 🔴 **前提**：2a 的清点循环已经跑完（他说了「没有了」或「跳过剩下的」），你手上有一张**完整**的改动清单。
 > 🔴 **这一步不是拒绝他的需求** —— 需求全部收下了，这里只决定**怎么跑最省**。措辞要是「怎么排最划算」，不是「一次只能改一处」。
...
5. **🔴🔴 运镜（镜头会不会动）—— 独立一题，问完机位马上问，绝不省（2026-08-24 补）**
...
6. **🔴🔴 背景要不要「活」—— 独立一题，选了换背景就一定要问（2026-08-24 补）**
...
7. **🔴🔴 背景要不要虚化一点（景深）—— 独立一题，换背景就一定要问（2026-08-24 补）**
...
→ 换背景时**必须**额外写云／树／光／远处有人（见 B-1 第 6 题的句子表）；远景人物一定带 `far in the background … small, distant, softly out of focus, mostly facing away`。
