...
> **成品归档（做完必做，禁找任何借口）：** 每个成品（图/视频/音乐/PPT）做完，**三处都要落，一处都不能跳**：① 本地成品 folder 的**对应子路径**（`<内容类型>/<主题>/<漏斗层>_<受众>/`、文件名 `<序号>-<STAGE>-<受众>-<标题>`、缺的子文件夹就建 · 结构见 01-branding「📁成品 folder 结构」）；② **贴进 Notion**（对应立项页的「成品」块 + 改 Status）；③ **`rclone copy` 传进用户 Google Drive 成品 folder**。**②③ 只要有一处没做，就是没做完 —— 绝不说「传不了 / 太大 / 浪费算力 / 你自己上传 / 你自己拖」这类借口。**
>
