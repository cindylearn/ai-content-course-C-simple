...
> 1. 🇲🇾 全中文（马来西亚华语）— 共鸣最直接，但 Suno 咬字可能糊，也许要多生几次
> 2. 中英混（主歌中文讲故事、副歌英文钩子）— Suno 唱英文更稳
> 3. 全英文
> （Bahasa Malaysia / 方言 → 走「其他」）
...
- **② 品牌歌 / jingle（有歌词有唱）** — 能记住的主题曲，目标「被记住」。
BGM 混人声会跟口播打架；品牌歌没歌词就不是歌。定方向再往下。

—— **路 A · 垫底 BGM** ——

### Step 2 · 定"感觉"（抢戏就是失败）
视频已有一个主角（讲话的人）。选 BGM 唯一标准不是「好不好听」，是「**有没有让讲话更好听**」。
- **Discover 是 prompt 图书馆，不是选歌的**：每首点「+ Show full styles」看作者完整 prompt 原文 —— 只偷**写法**不抄歌（音轨是别人的、且那池全有人声会打架）。标签骗人、完整 prompt 不骗人。让用户贴原文给你。
- **留 / 扔 / 加**：留 cinematic/orchestral/力量感/现代感；扔 人声/合唱/一开场就炸/体育场鼓/euphoric；加 无人声/给人声让路/BPM/时长。把「爆」换「稳」（`held back`/`restrained`）。

### Step 3 · 写 BGM prompt 五成分（少一个就靠运气）
1. **用途+格式**（`short-form vertical ad`）—— 漏了给你写一首完整的歌
2. **具体乐器**（`muted electronic pulse, plucked synth arpeggio…`）—— 含糊词每次都不一样
3. **调性含否定**（`professional and confident, not hype or cheesy`）
4. **给人声让路**（`no vocals` + `no melody that competes with speech` + 低中频挖掉）—— **最容易漏、最要命**（是频率打架不是音量）
5. **BPM + 时长**（`steady 100 BPM, builds gently, 85 seconds`）

### Step 4 · suno.com 出歌（免费可做）
① 登录 ② Create → 左上 `Simple` 切 `Advanced` ③ Styles 贴五成分、Lyrics 留空 ④ 模型选 **`v4.5-all`（Best free model）**，v5.5/v5/v4.5+/v4 全是 Pro ⑤ Instrumental 开 ⑥ Create → **弹 Google 人机验证，必须真人点**（所以"全自动出歌"不存在）⑦ 出 2 首 ⑧ **用户试听挑一首**（AI 没耳朵），不对说哪里不对（太吵/开头炸/咬字糊/抢戏）→ 改 prompt 重生。**核心分工：你写 prompt，用户当耳朵。**

### Step 5 · 垫进视频（用自动闪避，别手动压音量）
进剪辑软件靠轨道角色自动让路：人声轨=anchor、BGM=follower（讲话时音乐自动缩、不讲回来）。三个坑：① 别压两次（开了闪避就别再手动压）② 声音不对**先查 muted 再查音量** ③ 人声常在视频轨内嵌音频，闪避挂真正有人声那轨。（例 SUMA 84 秒竖版：BGM `-1.4dB` + follower `-19dB`）

—— **路 B · 品牌歌 / jingle（规则跟 BGM 全相反）** ——
...
- **别自吹**：改唱「我们看见你的难」（客户是英雄）；**别拿品牌歌当 BGM 垫口播**；短 jingle（5–15 秒）只用片头/片尾 logo 露出

### Step 7 · 收尾必存 Notion 音乐库（别跳）
Suno 一次给好几首，不归档 = 下次找不到、又重生花额度。Notion 建表每首一行：**标题**（歌名+版本时长）·**类型**（BGM/品牌歌）·**链接**·**用途**·**参考曲**·**状态**（草稿/可用/待改）·**备注**。标清能不能用（免费 `v4.5-all` 完整=可用；v5.5 预览 1 分钟片段要 Pro=待改，别误当成品发）。用户没这表就帮他建。
