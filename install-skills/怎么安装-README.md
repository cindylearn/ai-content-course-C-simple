# 怎么安装 + 怎么开始（9 个独立 skill）

这里面有 **8 个独立 skill**（00-07），每个都能单独调用。

## 一、装（一次）
- **一键**：在这个文件夹打开终端 → `bash 一键安装.sh` → 9 个全装进 `~/.claude/skills/`。
- **手动**：把 9 个文件夹逐个复制进 `~/.claude/skills/`。

## 二、怎么开始（触发 · 院长说的两种方式）

🔴 **第一次 / 新业务 → 先触发 `00 守门人`**，它会逼你先建好 Notion 地基（dashboard/folder/分类）再放行。
...
> 想一个 skill 走完全程 → 用 **07**；想一步一步来 → 从 **00** 开始，00→01→…→07。
