...
## 🌐 开工第一题（先于一切）· 这条文案用哪种语言？

> 🔴🔴 **一进来就先问这题 · 绝不跳过 · 绝不自己默认 —— 一张选项卡让学员选：**
> ① 🇲🇾 **马来西亚华文**（中文为主、自然掺一点 English / Malay / 方言，最本地口气）【默认 · 推荐】· ② 规范中文 · ③ English · ④ Bahasa Malaysia · ⑤ 中英 Rojak / 方言混（前 4 做卡、第 5 放「其他」）。
> 🔴 **SUMA 受众 ＝ 马来西亚华人 —— 默认就往 🇲🇾 马来西亚华文写**（接地气、像本地人讲话），**别写成生硬的规范中文 / 大陆腔**；但仍要问、让学员确认。选定 ＝ 这条文案就用它（🔴 你跟学员**对话仍一律用中文**）。业务 md 记了也要一句确认，别跳过。

---

## 📐 排版铁律 · 文案长什么样（🔴 出文案前先把这条刻在脑子里，别吐一坨字）

> 🔴🔴 **绝不输出「一大坨没分段、没 emoji 的字」** —— 那是没 follow SOP，直接重写。每条成品文案都必须长这样：
>
> - **① 分段**：**每一拍空一行**（真换行，别写 `\n` / `nn` / 挤成一段）。一句一段或两句一段，手机上读起来一小块一小块，不是一整面墙。
> - **② 标题**：第一行是**带钩子的强标题**，末尾放 **1 个** emoji。🔴 **绝不用【】方括号**（不写「【…】」）—— 直接把那句钩子写出来。🔴 **钩子里要直接带出核心产品/服务的那个关键词/概念**（卖 dashboard 就直接讲 dashboard、卖落地页就讲落地页），别只戳沾边痛点 —— 详见下方「§二 · 钩子铁律」。
> - **③ 适当 emoji**：全文 **3–6 个**，点在关键处（痛点、转折、CTA），**别每行都放**、也别一个都不放。
> - **🇲🇾 加国旗 · 放最开头**：内容是**马来西亚华文**时（默认），🔴 **文案第一行、钩子标题的最前面放一个 🇲🇾**（如 `🇲🇾 同样两间公司，为什么…？📊`）—— 标明本地身份。**只放开头这一个，结尾 / 品牌行 / hashtags 都不放。** 选了 English / 规范中文就不放。
> - **④ 结尾**：品牌名 + CTA + 链接，最后一行 hashtags。
> - **⑤ 字数达标**：ToFu ≥250 / MoFu ≥300 / BoFu ≥200 字（写完数一遍中文字数）。
>
> 📎 **照抄本 skill §五 三个示范的样子**（宠物美容 / 补习 / 美容院）—— 那就是分段 + emoji + 字数都对的成品长相。**每次出文案前，先想「我这条排出来像不像 §五 那样一小块一小块」，不像就重排再输出。**
...
> 🔴🔴 **钩子铁律 · 钩子里要直接带出核心产品/服务的「那个关键词/概念」（每条都守）：** 不是只戳一个沾边的痛点绕圈子 —— **核心产品/服务那个词（或它最核心的动作/结果）要直接出现在钩子里**，一看就知道这条在讲什么。
> - ✅ 卖 **dashboard / AI 帮你看数字** → 钩子直接讲 **dashboard / 一页看懂公司**（「为什么有的老板手机上有个自己会更新的 dashboard，你还在群里问？」），别只写「群里没人回」这种沾边痛点。
> - ✅ 卖 **AI Landing Page / landing page** → 钩子直接带 **落地页 / 会成交的页面**。
> - ✅ 卖 **补习** → 钩子直接带 **补习**（「同样是补习，为什么…」）。
> - ✅ 卖 **数字人口播** → 钩子直接带 **数字人 / 一个"你"替你天天上镜**。
> - ❌ 只戳沾边痛点、核心那个词没出现（「做生意好累」「竞争好大」）＝废钩子。
> 🔴 动笔前先读业务 md 的「**一句话生意 + 主打产品/服务**」，锁定**核心那个词**，钩子第一句就把它带出来。
...
## 二·五、钩子库 · 真实对标广告拆出来的「能用的写法」（换成你的行业）

> 这些是**市场上真在跑的本地广告**提炼出来的 hook 句式（美业 / 补习 / 电商 / 个人 IP / SaaS 都在用）。**照抄结构、换你的行业和数字**就行。**海报大标题、caption 第一行、视频前 3 秒都用它们。**

**① 结果量化型**（把结果变成一个具体数字 —— 越具体越可信）
> 「一键生成 **30 秒** 带货视频」·「以前 **6 小时**，现在 **60 分钟**」·「1 个月冲到 **100 万**流量」·「数学从 **40 分**到 **72 分**」·「一次配套省下 **大半年** 护理钱」
> 👉 写法：**动词 + 具体数字 + 结果**。数字要真（你交付得出来的），不是吹。

**② 对比反差型**（使用前 vs 使用后 / 别人 vs 你 —— 差距一眼看见）
> 「找达人，好费时又费力！→ AI 一键批量找达人，高效又轻松！」·「学完只能自己用？→ 贴你的 logo 卖给客户」·「同样是补习，为什么有的孩子越补越怕，有的越补越敢举手？」
> 👉 写法：**先说旧方式的痛（❌ 一条条列），再说你的新结果（✅ 一条条列）**。海报左右分栏最好用（见 03 版式库「对比图」）。

**③ 稀缺倒数型**（具体数字 + 时间线 —— OK，但别造假）
> 「**LAST CALL · 最后 10 位**」·「限时试用价 **RM3,500**」·「这个配套价**只到月底**，做满就收」·「下一个 intake 快开课了，赶快报名」·「今天 **$27**」
> 👉 写法：**真名额写数字、真截止写日期**。⚠️ 名额/截止必须是**真的**——报了名还能报＝假倒数，会被识破、伤信任。

**④ 身份认同 / 立场型**（击中「我想成为的那种人」）
> 「一个人 + AI 也能跑完整生意」·「女人有底气的能力」·「不是副业课，是一条走向高收入的职业路线」·「做自己，就是流量」
> 👉 写法：**说出受众渴望的身份**，不是说产品功能。

**⑤ 痛点戳心型**（先说出他心里那句话）
> 「每个平台都学一点，最后脑子一团乱，实际操作又不会」·「找达人，一个一个搜、复制粘贴到 Excel，每天花好几个小时」·「想到才做一次脸，做做停停，皮肤永远回不来」
> 👉 写法：**把他每天真实的糟心场景描出来**，让他「对，就是我」。

**⑥ 揭秘 / 反直觉型**（打断惯性、勾起好奇）
> 「别再学 AI 工具了 —— 真正高手学的是营销战略」·「会做项目的人，永远不缺赚钱机会」·「成绩上不去，不是不够努力」
> 👉 写法：**推翻一个大家默认的想法**，再给新答案。

> 🔴 **选钩子＝选角度**：一条内容挑 **1 个**主钩子起头，别把六种全塞进一张海报。ToFu 多用 ①②⑤⑥，MoFu 多用 ①②④，BoFu 多用 ①③（算账＋倒数）。

---

## 二·六、🔴 真实性红线 —— 数字可以有，牛不能吹

> 📌 **量化结果 ✅ · 稀缺倒数 ✅ · 具体数字 ✅ —— 但一切必须是真的、交付得出的。** 分清「有力」和「浮夸」：
>
> **✅ 可以（有力、可信）：**
> - **量化结果**：30 秒出片 / 6 小时→60 分钟 / 40 分→72 分 / 省下大半年的钱 —— 只要你**真做得到**。
> - **稀缺倒数**：最后 X 位 / 只到月底 / 限时价 RMxxx —— 只要**名额和截止是真的**。
> - **具体数字**：价格、配套内容、时长、件数 —— 越具体越可信（模糊反而像心虚）。
> - **学员 / 客户见证**：✅ **见证是很强的角度、鼓励用**（尤其 MoFu「学员见证」钩子）—— 用**真客户的真话 / 真结果**；一时没有 → 先放占位 `<真实见证待补>` 让用户课后填**真的**。**见证本身不是红线。**
>   - 🔴 **见证要真实可信、别夸张**：语气像真人随口说、结果**温和合理**（"排队少了""客人回头率高了点"）；**绝不用太夸张的数据 / 奇特效果**（"三天暴涨十倍""一用就爆单""立刻满房")——越夸张越假、越没人信，反而毁掉说服力。
>
> **❌ 绝不可以（浮夸、不实、伤信任）：**
> - **暴富承诺**：「月入过万」「躺着赚」「三天翻倍」「保证成功」「包你爆单」—— 尤其是「**不用努力 / 零基础立刻 / 睡后收入**」这类。
> - **假倒数**：写「最后 10 位」但其实随时能报；写「今天截止」天天都是今天。
> - **无依据绝对化**：「全马第一」「100% 有效」「永远不会失败」——除非你有证据，否则别写。
> - **把编造的假见证当真的发**：用**不存在的人 / 编的原话**冒充真客户、伪造 before→after 截图当真事发布。（🔴 只是这一条 —— **正常写学员 / 客户见证内容没问题**，用真的或留占位即可。）
>
> 🎯 **一句话判准**：这个数字 / 承诺，**客户来问你、你敢当面兑现吗**？敢＝写；心虚＝改成你敢兑现的说法。**卖结果，不卖幻觉。**
...
> - **分段**：每拍之间空一行；真换行，别写 `\n` / `nn`。
> - **标题不用【】**：钩子标题直接写出来，🔴 **绝不套「【…】」方括号**。
> - **适当 emoji**：标题 1 个 ＋ 关键处点缀，全文 3–6 个。**🇲🇾 马来西亚华文时，把一个 🇲🇾 放在文案最开头（钩子标题最前面），别放结尾/hashtags。**
...
> 🎯 **定位铁律（写文案前先锁）：文案卖的是「你(user)自己的服务/产品」给「你的买家」—— 别写成「买家那个行业」的科普。** 三层：① 谁卖 = user · ② 卖什么 = user 服务/产品（＝文案主角）· ③ 卖给谁 = 买家（＝受众）。用买家定**称呼 / 痛点**（"开补习中心的你"对），但通篇在讲**你能给买家什么结果**（你的服务 / 成效 / 承诺）。服务型（agency / 自由职业 / 顾问 / SUMA 自己）尤其：主线是"**我帮你达成 X**"，买家行业只当举例证明。🔴 先读业务 md 的"一句话生意"确认「这条在卖 **[你的服务]** 给 **[买家]**」再动笔。
...
## ✍️ 写一条文案 · 照 Step 1–5 走（一步做完再下一步）

### Step 1 · 判这条是哪一层（先定层，才知道卖什么、写多长）
- **ToFu 觉醒** — 戳痛点/趋势，让人「对，就是我」；不硬卖、不放 offer/价。**≈250–400 字**。
- **MoFu 考虑** — 拆方法 / 给证据 / 讲 USP，证明「这方法行」。**≈300–500 字**（最长最实）。
- **BoFu 决策** — 算账 ROI + 紧迫 + offer。**≈200–350 字**。
> 字数是**下限不是上限**，宁长别短，绝不缩成 1–2 句 caption。把场景/情绪/对比铺开才够长。

### Step 2 · 照节拍表写骨架（每拍绑大师/NLP/需求；完整三张表见「参考 · 文案框架 §四」）
卖你自己的产品/服务给你自己的顾客，**AI 全程隐形**；只有生意本身 = 卖 AI / 代运营，才带出工具组。**绝不 import「教你用 AI 做内容」这种 SUMA 的角度。**
- **ToFu（9 拍）**：钩子 → 痛点场景(具体画面) → 二次扎心 → 正名(不怪客户) → 后果/对比 → 换框 → 好消息=你的产品怎么解决 → 愿景收口 → CTA
- **MoFu（9 拍）**：钩子(揭秘/见证) → 点破误区/正名 → 方法一 → 方法二 → 证据(before→after) → 换框 → 为什么选我(USP) → 收口 → CTA
- **BoFu（6 拍）**：钩子(算账/限时) → 算账对比 → offer 内容 → 消除顾虑 → 紧迫(名额有限) → 愿景 + CTA
每拍展开 1–2 句真内容，填满自然达字数；缺拍 / 没展开 = 字数不够 = 重写。

### Step 3 · 每拍叠大师 + NLP + 需求（每条至少 2–3 个大师，说得出用了谁）
**Blair Warren 为处境正名**(灵魂,几乎每条:「不是你不够 X,是 Y」) · **LF8** 戳本能(怕淘汰) · **AIDA/PAS**(PAS→ToFu·AIDA→BoFu)当骨架 · **Cialdini**(社会认同=见证 / 稀缺=名额有限) · **Belfort** Before→After 卖 ROI · **Duarte** 客户是英雄你是向导。
NLP 手法叠 1–2 个（全文见「参考 · NLP 技巧」）；需求戳 1 个（见「参考 · 6 大人性需求」）。

### Step 4 · 过护栏（合规红线，写死）
- **完整品牌名一字不差**，每条 1 次（常放 CTA）
- **卖你自己的产品/服务、AI 隐形**（生意=卖 AI/代运营才带工具组）
- 人物一律本地 · 无口头禅、要多要实 · 换行用**真换行**（别写 `\n` / `nn`）
- ⚠️ 价格/优惠/名额**按你自己生意来**；「不写价 / 不写免费 / 不写名额数字」只是 **SUMA 自己招生**的房规，别套到学员生意上。

### Step 5 · 输出到 Notion「文案」列 →（🔴 别在这里 confirm / 改 Status，接着做图/视频）
照 Step 2 节拍表填满每一拍、**分段 + 适当 emoji**（标题 1 个 + 关键拍点缀，全文 ≈3–6 个，别每行都放）。
**字数硬达标**：ToFu ≥250 / MoFu ≥300 / BoFu ≥200 字 —— 写完数一遍中文字数，不到下限就把某几拍展开更细再数，达标才输出。
写完自检：卖的是自己产品吗（没 import 别人角度）？AI 隐形吗？字数够吗？分段 + emoji 了吗？每拍说得出套了哪个大师 / NLP / 需求吗？**任一不行 = 重写** → 回填「文案」列。
🔴 **文案回填后不要在这里问「OK 吗 / 要不要生成」、也不要把 Status 改成 Proceed to Generate** —— 一条内容要**文案 + 图（或视频）都齐**才算 ready。**紧接着触发 `03-image`（说「帮我出图」）问图的设计问题、写图 prompt**；等文案 + 图 prompt 两样都进 Notion，**才**一次性 → 教用户去 Notion check → OK → 改 Status `Proceed to Generate` → 再问「开始生成 or 继续想新 idea」。完整顺序见本 skill 顶部「🧩 做『一条内容』的完整顺序」。