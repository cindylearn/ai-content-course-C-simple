...
> - **② 标题**：第一行是**带钩子的强标题**，末尾放 **1 个** emoji。🔴 **绝不用【】方括号**（不写「【…】」）—— 直接把那句钩子写出来。
> - **③ 适当 emoji**：全文 **3–6 个**，点在关键处（痛点、转折、CTA），**别每行都放**、也别一个都不放。
> - **🇲🇾 加国旗**：内容是**马来西亚华文**时（默认），全文放**一个 🇲🇾 国旗** —— 放**品牌名 / CTA 那一行**（如 `<品牌名> 🇲🇾`）或 hashtags 里（`#马来西亚 🇲🇾`），标明本地身份（算进上面 3–6 个 emoji 里）。选了 English/规范中文就不放。
...
> - **适当 emoji**：标题 1 个 ＋ 关键处点缀，全文 3–6 个。**🇲🇾 马来西亚华文时全文放一个 🇲🇾 国旗**（品牌/CTA 行或 hashtags，如 `#马来西亚 🇲🇾`）。
