...
> 🔴 **第一题（必问 · 排在下面 11 题最前面 · 实测踩过的大坑）：这支片是「原始素材」还是「你已经剪过的」？**
> → ① **原始素材**（没剪过 · 可能有口误 / 停顿 / 赘词）：可以**删赘词、重排节奏、切顺语流**。
> → ② **已经剪过的**（你已排好语流 / 节奏）：🔴 **绝不再删赘词、绝不切碎语流、绝不动已排好的 timing** —— 只做**叠加层**（加字幕 / 重点字 / 颜色 / 音效 / BGM / 片尾），保留原剪辑的断句和节奏。
> ⚠️ **为什么必问**：曾经没确认就默认「删赘词 = 正常口播节奏」，把已排好的语流切碎，导致后面**字幕断句反复修不好**。🔴 **没确认『原始 / 已剪』之前，绝不删词、绝不动时间轴。**
...
10. **整体节奏？**（🔴 只有第一题选「原始素材」才可重排；选「已剪过」→ 必选④，绝不重切）→ ① 快剪带感（默认·仅原始素材）· ② 正常口播节奏（仅原始素材）· ③ 慢稳 · ④ **跟原片节奏、不重切、不删词**（已剪过必选）
11. **片头片尾？** → ① 片尾放 logo + 一句 CTA 卡（默认）· ② 只片尾 logo · ③ 都不要

> 比例默认 **9:16 竖版**（不用问）。答完**「第一题·原始/已剪」+ 这 11 题** → 记进 Notion 立项页「剪辑需求」→ 才照 `CHATCUT-SKILL` 手册开剪。**没问就剪 = 违规重来；没确认『原始/已剪』就删词切时间轴 = 违规重来。**
