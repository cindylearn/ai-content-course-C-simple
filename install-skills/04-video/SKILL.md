...
**主讲人只 mention：** 想全片同一个人 / 真人本尊 / 锁定特定 AI 脸 → 才去 `suma-ai-04-video`（自己样子的数字人 或 九宫格锁一个）。先 test 不用 05。
...
- **开工只问两件（一题一张选项卡，别默认）：**
  1. **出镜的人是哪种？（三选一）**
     - **① AI 人**（default）— 照片级真人数字人，AI 生成、不是真人本尊。要**全片同一张脸** → 见 Step 3「锁定出镜的人」九宫格选一张锁定；只 test 一支就直接照脚本 persona 出。
