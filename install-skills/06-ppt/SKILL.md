...
> 🔴🔴 **一进来先问这题 · 绝不跳过 · 绝不自己默认 —— 一张选项卡让学员选：**
> ① 🇲🇾 **马来西亚华文**（中文为主、自然掺一点 English / Malay / 方言，最本地口气）【默认 · 推荐】· ② 规范中文 · ③ English · ④ Bahasa Malaysia · ⑤ 中英 Rojak / 方言混（前 4 做卡、第 5 放「其他」）。
> 🔴 **SUMA 受众 ＝ 马来西亚华人 → 默认就往 🇲🇾 马来西亚华文走**（接地气、像本地人讲话），别写成生硬的规范中文 / 大陆腔；但仍要问、让学员确认。🔴 **你跟学员对话仍一律用中文**。业务 md 记了也要一句确认，别跳过。
...
> 🔴🔴 **06-ppt 开工先问这两题 —— 每题都是一张独立 `AskUserQuestion` 选项卡、停下等她点，🔴 绝不自己默认 / 推断答案、绝不用一句陈述代替卡（在语言 / 受众之前 · PPT 专属）：**
> **① 这份 PPT 是关于什么？给哪个业务 / 主题做？**（一张卡）🔴 **绝不默认＝业务 md 里那个生意！** PPT 常常是**一次性、针对某个具体主题**的（一个客户提案 / 一个产品 / 一份汇报 / 一个课题），主题可能跟你的内容营销生意完全不一样。选项：`① 就是我业务 md 的生意 · ② 别的主题 / 业务（打字说清是什么、给谁）· ③ 其他`。**读到 md 也要问、等她选，别读了就默认。**
> **② 这份 PPT 做什么用（目的 / 场景）？**（另一张卡）🔴🔴 **就算 ① 的答案里已经暗示了用途（如她答「内部汇报」），Q2 也要单独用一张卡问出来让她选 / 确认 —— 绝不自己默认成某个框架（别写「内部汇报默认走 BLUF」这种）。** 选项：`销售提案·成交（Monroe/OPA）· 客户汇报·决策（BLUF/SCQA）· 教学·培训（BOPPPS/4MAT）· 投资路演 / 招商 / 内部汇报 · 其他`。
> 🔴 **节奏：① 问完 → 停下等她答 → 才出 ②；② 答完 → 才进语言 / 受众。绝不两题合并、绝不把答案默认进去、绝不「记下 X → 顺手替她定 Y」。**
>
> **统一提问顺序（固定，别临时调换）：** 第一题 · 🌐内容语言 → 第二题 · 做哪个受众 → 第三题 · 哪个漏斗层（ToFu/MoFu/BoFu）→ 第四题起 · 本 skill 专属。内容语言 / 受众 / 漏斗层若业务 md 已记 → 一句确认、不重复问。🔴 **06-ppt 例外：上面「PPT 主题 + 用途」两题最先问，且主题这题即使 md 有也要问（因 PPT 主题≠固定生意）。**
...
> 🎯 **定位铁律（做卖点页前先锁）：每一页卖的是「你(user)自己的服务/产品」的价值给「你的买家」—— 别做成「买家那个行业」的介绍。** 三层：① 谁卖 = user · ② 卖什么 = user 服务/产品（＝每页主角）· ③ 卖给谁 = 买家（＝受众）。页面主体（大标题 / 图表 / 案例 / 价格）都在证明**你能交付什么结果**；买家行业只当场景 / 举例。服务型（agency / 顾问 / SUMA）用**成效数据 + 作品样本 + before→after**撑价值。🔴 先读业务 md 确认「这份在卖 **[你的服务]** 给 **[买家]**」再排版。
...
## 🖥️ 做卖点页 / PPT · 照 Step 1–5 走（一步做完再下一步）

### Step 1 · 先套身份 + 诊断（结构没定，绝不先谈美化）
🎩 CMO · 增长黑客 · 演示架构大师。做 PPT 前先诊断：这份 deck 的**目标**、**受众**、要用户**看完做什么动作**。

### Step 2 · 判三大场景，套对应框架（先判属于哪种）
- **🅰️ 销售 / 提案（说服成交）** — Monroe 动机序列（注意→需求/痛点→满足→愿景 Before vs After→行动）/ OPA（客户原话→计划→前后 ROI）。从**客户痛点**开局别讲公司介绍；为处境正名（错的是方法不是他）；卖 ROI 不卖功能；让客户**自己说服自己**（诊断清单「你中几项」/自己算 ROI/价值堆叠 `Worth RMx` 累加再亮低价/顾虑前置/稀缺+风险反转）。
- **🅱️ 课程 / 教学（知识内化）** — BOPPPS（导言→目标→前测→参与→后测→总结）/ 4MAT（Why→What→How→What If）。一页一概念控认知负荷；必设互动；开场先小测让学员发现「我不会」再教。
- **🅲 汇报 / 数据（辅助决策）** — BLUF（结论置顶）/ SCQA（背景→冲突→疑问→解答）。每页标题=**带结论的陈述句**；MECE + 数据墨水比、删装饰；点出数据背后**洞察**别只扔图表。
- 说服建立在**真实价值**：不夸大 / 不虚假承诺 / 不伪造数据；`Worth` 标价要有依据。

### Step 3 · 必问：要不要放照片（做 PPT 前问，一题一张卡）
- **Q1 要放照片吗？** 要→Q2；不要→用图表 + 图标 + 品牌色排版，跳过。
- **Q2 照片从哪来？** ① 自己有→Q3 ② AI 生成→去 `03《AI产品图》` 出好再插 ③ 网上找→提醒版权（只用可商用/有授权）。
- **Q3 怎么给？** ① 拖进对话 ② 本地路径 ③ 公开链接（Drive/图床）。
- 缺哪张标「这里留位放照片」，别用假图糊弄。

### Step 4 · 出品质量（别出半成品）
- **一页一卖点/概念**：少字大字，绝不整页文字轰炸
- 该放真实照片就放（封面 hero/案例成果/头像/产品），别留空位
- **数据用图表不用表格**；一张图一个结论；配图/图表优先于大段文字
- **内容要满要细**：每页除标题要有支撑要点/数据/例子，别只放口号
- **套品牌**：色 hex + logo + 字体统一（SUMA `#28c2e7`/`#0f75bc`/`#fcb040`），别用模板默认

### Step 5 · 怎么做出来（两条路线 · 实测）
- **A · HTML → PDF（快、好看、不可编辑）**：Claude 按品牌色写 HTML 幻灯（一页一 slide、16:9）→ Chrome 无头 `chrome --headless --print-to-pdf` → 直接发/投影的成品。
- **B · 可编辑 `.pptx`**：Claude 生成 `python-pptx` 脚本 → `.pptx`（要交给别人改时用）。
- **怎么 prompt**：给它 ①内容大纲(问题→方案→证据→行动) ②品牌色+logo+字体 ③「一页一卖点、图表>大段字、内容要满」→ 出整套；不满意精确指「第几页改成…」。

### ⚠️ 踩坑速查
- 视频/图里 show PPT：用图表/形状/干净英文占位，**别出乱码中文**（同一条护栏）。
- 卖点数字别违规（价格/名额），例：不写「40 席」用「名额有限」。
