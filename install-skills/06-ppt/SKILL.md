...
🔴 先判场景（销售=Monroe/OPA · 教学=BOPPPS/4MAT · 汇报=BLUF/SCQA）套对应大师框架，结构定了再美化。
🔴 **必问要不要放照片**（选项卡：Q1 要不要 → Q2 从哪来[自己有 / 去 `03` 生成 / 网上找⚠️版权] → Q3 怎么给）。
🔴🔴 **硬规则（完整见 README）：**
- **一页一个卖点/概念**，少字大字，别整页文字轰炸；每页要有支撑要点/数据/例子，别只放口号。
- **数据用图表不用表格**；配图/图表 > 大段文字。
- **套品牌**（品牌色 + logo + 字体统一，别用模板默认）；缺照片就标「留位放照片」，别用假图。
- 🔴 视频里 show PPT / 出图里的 PPT：**干净英文占位、别出乱码中文**（同一条护栏）；卖点数字**别写价格/名额**。
两条出片路线：A `HTML→PDF`（成品）/ B `python-pptx`（可编辑）。
