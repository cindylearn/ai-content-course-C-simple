...
### 第 4 步 · 品牌地基（新业务必做 · 一次一题、按固定顺序问，问完复述确认）
🔴 **顺序写死（跟 `01-branding` 的 7 题一致，别跳、别调换）：**
1. **一句话说清生意/品牌做什么**
2. **品牌名 / 主打产品 / CTA 链接**（内容最后导去哪）
3. **目标受众 3–5 个**（想不出帮他 brainstorm）
4. **每个受众的 persona**：气质 / 出现场景（人物一律本地人）
5. **合规红线**：哪些词不能用（按他自己生意，不是套 SUMA 房规）
6. **内容路线**：① 个人 IP ② 卖产品/服务（核心分叉，必选一条）
7. **命名格式**：选一个之后全统一
→ 全部写进业务 md。（完整范例见 `01-branding` Step 2 + 本文件夹「参考 · TEMPLATE」）

🔴 **接着问 logo（一题一张卡，问齐三种）：有没有品牌 logo？有哪个给哪个 —— logo 绝不让 AI 画（会画出假的）：**
- ① **深色版**（深色图案，放**浅色背景**用）
- ② **浅色 / 白色版**（放**深色背景**用）
- ③ **透明背景 PNG**（最关键 —— 没透明底合成会带白框、难看）
→ 收到的 logo 存进成品 folder，路径写进业务 md；成品图会自动在顶部叠色带放 logo（色自适应）。三种都没有？先记着，让他课后补，别卡住流程。
